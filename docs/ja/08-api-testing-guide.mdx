...
このガイドでは、3つのAPIセキュリティテストアプリケーション全体にわたるすべてのAPIエンドポイント、意図的な脆弱性、および攻撃ペイロードをカタログ化しています。F5 XC API保護プロファイル開発のためのトラフィック生成パターンの構築にご活用ください。
...
| `me(token)` | Query | なし | JWTによるユーザー取得（偽造に対して脆弱） |
| `systemHealth` | Query | なし | ヘルスチェック |
| `systemUpdate` | Query | なし | 低速クエリ（約82秒、DoSベクター） |
| `systemDiagnostics(cmd)` | Query | admin/password | ホワイトリストされたOSコマンドの実行 |
| `createPaste(title, content, public)` | Mutation | なし | ペーストの作成（contentを介したXSS） |
...
**フィールドサジェスチョン（タイプミスで有効なフィールドが漏洩）:**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
  -d '{"query":"{pastes{titl}}"}'
...
**uploadPasteを介した任意ファイル書き込み（パストラバーサル）:**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**認証:** フォームエンコードPOSTによるJWT（`/restaurant/token`宛）
...
| `/restaurant/profile` | PATCH | あり | 任意 | **BOPLA**（ロールのマスアサインメント） |
| `/restaurant/users/update_role` | PUT | あり | 任意 | **BFLA**（ロールエスカレーション） |
...
**Customerとしてメニュー項目を削除（Employee以上が必要）:**
```bash
...
**メニューのimage_urlを介して内部エンドポイントを探査（Employeeロールが必要）:**
```bash
# まずBOPLAを使ってEmployeeにエスカレーションしてから:
...
| `/identity/api/auth/login` | POST | なし | JWTトークン（アルゴリズム混乱） |
...
| `/identity/api/v2/user/videos/{id}` | PUT | あり | **マスアサインメント**（コマンドインジェクション） |
...
| エンドポイント | メソッド | 認証 | 脆弱性 |
|---|---|---|---|
| `/community/api/v2/community/posts` | GET | あり | **データ露出**（vehicle_id、メールの漏洩） |
...
| `/workshop/api/shop/orders/{id}` | PUT | あり | **マスアサインメント**（status, quantity） |
...
curl -sf ${CRAPI}/identity/api/v2/user/videos \
  -H "Authorization: Bearer ${TOKEN}" | python3 -m json.tool
# レスポンスのconversion_paramsフィールドを確認
```

#### チャレンジ6 — DoS: 整備士連絡機能を介したレイヤー7攻撃
...
#### チャレンジ8 & 9 — マスアサインメント: 注文操作による無料商品の取得
...
| **API2: 認証の不備** | JWTの偽造（`me`クエリ） | JWT弱い秘密鍵 (97952) | OTPブルートフォース (v2)、JWTアルゴリズム混乱 |
| **API3: BOPLA** | — | `PATCH /profile`（ロールエスカレーション） | 動画conversion_params、整備士メール露出 |
| **API4: リソース消費** | バッチ、再帰、エイリアス、フィールド重複によるDoS | レスポンス長を利用したユーザー列挙 | `contact_mechanic` repeat_request DoS |
| **API5: BFLA** | — | `DELETE /menu`, `PUT /users/update_role` | 一般ユーザーとして `DELETE /admin/videos` |
| **API6: マスアサインメント** | — | `PUT /orders`（status, quantity） | 注文のstatus/quantity、動画conversion_params |
...
| **API10: 安全でないAPI消費** | — | — | （SSRFを介した間接的利用） |
...
攻撃パターンのテスト前に、通常のAPI動作を確立するための正規トラフィックを生成します。
...
**認証バイパステスト:** JWTトークンの偽造、OTPのブルートフォース、非推奨APIバージョンの使用、アルゴリズム混乱。

**SSRFテスト:** `importPaste`、`image_url`、`mechanic_api`パラメータへの内部URL。

**DoSテスト:** GraphQLバッチ/再帰/エイリアス攻撃、高カウント値での`repeat_request_if_failed`。

### フェーズ3: ステートフルな攻撃チェーン

一部の攻撃では、状態を持つマルチステップシーケンスが必要です:

1. **RESTaurantエスカレーションチェーン:** 登録 → トークン → PATCHでロールをChefに変更 → 再認証 → コマンドインジェクション
2. **crAPI完全フロー:** サインアップ → メール検証（MailHog） → ログイン → 車両追加 → 整備士連絡（SSRF） → 注文操作
3. **DVGA偵察から攻撃まで:** イントロスペクション → systemDiagnosticsの発見 → 管理者認証情報のブルートフォース → OSコマンド実行

### レートの推奨事項

| パターン | リクエスト/秒 | 継続時間 | 備考 |
|---|---|---|---|
| ベースライン（アプリごと） | 10-50 | 5分 | 通常トラフィックのフィンガープリントを確立 |
| BOLA列挙 | 100-500 | 2分 | 連番IDスキャン |
| OTPブルートフォース | 1000以上 | 発見まで | 最大10,000試行（4桁） |
| GraphQL DoS | 10-50 | 30秒 | 各リクエストはサーバー側で高負荷 |
| インジェクションファジング | 50-200 | 5分 | リクエストごとにペイロードを変更 |
| SSRFプロービング | 5-20 | 2分 | 低速; 各リクエストがサーバー側HTTPリクエストを発生 |
