...
title: 検証
description: >-
  9つのアプリケーションエンドポイントすべてに対するcurlスモークテスト、Dockerコンテナのヘルスチェック、nginxステータスの確認、cloud-initログの調査、および一般的なトラブルシューティング手順
sidebar:
  order: 5
i18n:
...
これにより、9つのアプリケーションすべて、ヘルスエンドポイント、VAmPIの登録/ログイン、CSD Demoのデータ持ち出しラウンドトリップ、DVGA GraphQL、RESTaurant Swagger、ポート8888のcrAPI、nginx gzip、およびバージョン非表示が検証されます。

### Dockerコンテナのステータス

VMにSSHで接続し、41個のコンテナすべてが実行中であることを確認します:
...
アップストリームコンテナが準備できていないか、クラッシュしています:
...
Dockerイメージは約4.7 GiBを消費します。CDN負荷テスト時のディスク枯渇を防ぐため、アクセスログは無効化されています。多層防御としてlogrotateが設定されています（500 MiB上限）。ディスク使用量を確認してください:
