...
title: 検証
description: >-
  9つのアプリケーションエンドポイントすべてに対するcurlスモークテスト、Dockerコンテナのヘルスチェック、nginxステータスの検証、cloud-initログの確認、および一般的なトラブルシューティング手順
sidebar:
  order: 5
i18n:
...
cloud-initの完了まで5〜10分待ってから、以下のチェックを実行してください。
...
すべて `200` を返すはずです（DVWAはログインページへの `302` リダイレクトを返します）。
...
これにより、9つのアプリケーションすべて、ヘルスエンドポイント、VAmPIの登録/ログイン、CSD Demoのデータ流出ラウンドトリップ、DVGA GraphQL、RESTaurant Swagger、ポート8888のcrAPI、nginx gzip、およびバージョン非表示が検証されます。

### Dockerコンテナのステータス

VMにSSH接続し、41個すべてのコンテナが実行中であることを確認します:
...
エンドポイントが502または接続拒否を返す場合、cloud-initがまだプロビジョニング中の可能性があります:

```bash
ssh azureuser@${ORIGIN_IP} "cloud-init status"
```

- `status: running` -- まだプロビジョニング中です。数分お待ちください
- `status: done` -- プロビジョニング完了です。Dockerコンテナを確認してください
...
アップストリームのコンテナが準備できていないか、クラッシュしています:
...
Dockerイメージは約4.7 GiBを消費します。CDN負荷テスト時のディスク枯渇を防ぐため、アクセスログは無効になっています。多層防御としてlogrotateが設定されています（500 MiB上限）。ディスク使用量を確認してください:
...
メモリ上限に近づいているコンテナがないか確認します（OOMリスク）:
