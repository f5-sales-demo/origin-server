...
cloud-init が完了するまで 5〜10 分待ってから、以下のチェックを実行してください。
...
すべて `200` を返すはずです（DVWA はログインページへの `302` リダイレクトを返します）。

### 自動スモークテスト

リポジトリには 39 項目のスモークテストスイートが含まれています:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

このテストでは、9 つのアプリケーションすべて、ヘルスエンドポイント、VAmPI の登録/ログイン、CSD Demo のデータ送受信ラウンドトリップ、DVGA GraphQL、RESTaurant Swagger、ポート 8888 上の crAPI、nginx gzip、およびバージョン非表示が検証されます。

### Docker コンテナのステータス

VM に SSH 接続し、41 個のコンテナすべてが実行中であることを確認します:
...
エンドポイントが 502 または接続拒否を返す場合、cloud-init がまだプロビジョニング中の可能性があります:
...
- `status: done` -- プロビジョニングが完了しています。Docker コンテナを確認してください
...
Docker イメージは約 4.7 GiB を消費します。CDN 負荷テスト時のディスク枯渇を防ぐため、アクセスログは無効化されています。多層防御として logrotate が設定されています（500 MiB 上限）。ディスク使用量を確認するには:
...
メモリ制限に近づいているコンテナがないか確認します（OOM リスク）:
