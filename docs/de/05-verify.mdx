...
description: >-
  ...
  Docker-Container-Gesundheitsprüfungen, nginx-Statusüberprüfung,
  cloud-init-Loginspektion und häufige Fehlerbehebungsschritte
sidebar:
  order: 5
i18n:
...
Führen Sie diese Überprüfungen durch, nachdem Sie 5–10 Minuten für den Abschluss von cloud-init gewartet haben.
...
Alle sollten `200` zurückgeben (DVWA gibt eine `302`-Weiterleitung zur Anmeldeseite zurück).
...
Diese validiert alle 9 Anwendungen, Health-Endpunkte, VAmPI-Registrierung/-Anmeldung, CSD-Demo-Exfiltrations-Roundtrip, DVGA GraphQL, RESTaurant Swagger, crAPI auf Port 8888, nginx gzip und Versionsverschleierung.
...
Wenn Endpunkte 502 oder "Verbindung abgelehnt" zurückgeben, könnte cloud-init noch mit der Bereitstellung beschäftigt sein:
...
- `status: done` – Bereitstellung abgeschlossen, überprüfen Sie die Docker-Container
- `status: error` – überprüfen Sie die Logs unten
...
# Alle Container neu bauen und neu starten
ssh azureuser@${ORIGIN_IP} "cd /opt/origin-server && sudo docker compose build && sudo docker compose up -d"
```

### Speicherplatz

Docker-Images verbrauchen ungefähr 4,7 GiB. Das Zugriffs-Logging ist deaktiviert, um eine Festplattenerschöpfung bei CDN-Lasttests zu vermeiden. Logrotate ist als Tiefenverteidigung konfiguriert (500 MiB Obergrenze). Überprüfen Sie die Festplattennutzung:
...
Überprüfen Sie, ob Container sich ihren Speicherlimits nähern (OOM-Risiko):
...
Falls erforderlich, bauen Sie alle Dienste neu und starten Sie sie neu:
