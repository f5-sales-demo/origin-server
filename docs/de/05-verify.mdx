...
description: >-
  ...
  cloud-init-Loginspektion und gängige Fehlerbehebungsschritte
sidebar:
  order: 5
i18n:
...
Das Repository enthält eine 39-Punkte-Smoke-Test-Suite:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

Diese validiert alle 9 Anwendungen, Health-Endpunkte, VAmPI-Registrierung/Anmeldung, CSD-Demo-Exfil-Roundtrip, DVGA GraphQL, RESTaurant Swagger, crAPI auf Port 8888, nginx gzip und Version-Hiding.
...
Wenn Endpunkte 502 oder "Verbindung abgelehnt" zurückgeben, ist cloud-init möglicherweise noch bei der Bereitstellung:
...
Docker-Images belegen ungefähr 4,7 GiB. Access-Logging ist deaktiviert, um eine Festplattenerschöpfung bei CDN-Lasttests zu verhindern. Logrotate ist als Defense-in-Depth konfiguriert (500 MiB Obergrenze). Überprüfen Sie die Speicherplatzbelegung:
...
Falls erforderlich, alle Dienste neu bauen und neu starten:
