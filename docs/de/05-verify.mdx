...
description: >-
  ...
  Docker-Container-Integritätsprüfungen, nginx-Statusverifizierung,
  cloud-init-Protokollinspektion und häufige Fehlerbehebungsschritte
sidebar:
  order: 5
i18n:
...
Führen Sie diese Prüfungen durch, nachdem Sie 5–10 Minuten gewartet haben, bis cloud-init abgeschlossen ist.
...
Alle sollten `200` zurückgeben (DVWA gibt `302`-Weiterleitung zur Anmeldung zurück).
...
Diese validiert alle 9 Anwendungen, Integritätsendpunkte, VAmPI-Registrierung/Anmeldung, CSD-Demo-Exfil-Rundtrip, DVGA GraphQL, RESTaurant Swagger, crAPI auf Port 8888, nginx gzip und Versionsverbergung.

### Docker-Container-Status

Stellen Sie eine SSH-Verbindung zur VM her und überprüfen Sie, ob alle 41 Container laufen:
...
Wenn Endpunkte 502 oder „Verbindung verweigert" zurückgeben, kann cloud-init noch bereitstellen:

```bash
ssh azureuser@${ORIGIN_IP} "cloud-init status"
```

- `status: running` – wird noch bereitgestellt, warten Sie einige Minuten
- `status: done` – Bereitstellung abgeschlossen, Docker-Container prüfen
- `status: error` – Protokolle unten prüfen

### cloud-init-Protokolle
...
```bash
# Container-Protokolle prüfen (jede App hat 4 Instanzen: -1 bis -4)
...
Der Upstream-Container ist nicht bereit oder abgestürzt:

```bash
# nginx-Fehlerprotokoll prüfen
...
# Alle Container neu erstellen und neu starten
ssh azureuser@${ORIGIN_IP} "cd /opt/origin-server && sudo docker compose build && sudo docker compose up -d"
```

### Speicherplatz

Docker-Images verbrauchen ungefähr 4,7 GiB. Die Zugriffsprotokollierung ist deaktiviert, um eine Erschöpfung des Speicherplatzes bei CDN-Lasttests zu verhindern. Logrotate ist als Defense-in-Depth konfiguriert (500-MiB-Grenze). Speicherplatzbelegung überprüfen:
...
Auf Container prüfen, die sich ihren Arbeitsspeichergrenzen nähern (OOM-Risiko):
...
Falls erforderlich, alle Dienste neu erstellen und neu starten:
