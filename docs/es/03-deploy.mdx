...
description: >-
  ...
  demostración — 9 archivos terraform con resolución automática del
  implementador Azure AD, 25 salidas y aprovisionamiento cloud-init endurecido
  de 41 contenedores Docker en 9 aplicaciones.
sidebar:
  order: 3
i18n:
...
### Configuración de la base de datos DVWA
...
:::note
La base de datos SQLite de VAmPI se inicializa automáticamente mediante su script de entrada de gunicorn. Cada contenedor ejecuta `db.create_all()` al inicio, por lo que no se requiere intervención manual.
:::

## Conexión a componentes posteriores
...
|---|---|---|
| `public_ip` | Simulador CDN | `origin_server` (con prefijo `http://`) |
| `public_ip` | Simulador CDN | `origin_host` (con sufijo `:80`, sin esquema) |
| `public_ip` | F5 XC Origin Pool | Dirección del servidor de origen |

Continúe con [Applications](../04-applications/) para guías de uso o [Verify](../05-verify/) para pruebas de humo.
