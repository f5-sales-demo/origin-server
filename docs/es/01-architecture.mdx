...
Este componente reemplaza un servidor de aplicaciones de producción real con una VM construida a propósito que ejecuta aplicaciones vulnerables bien conocidas que activan reglas WAF, políticas de seguridad de API y detección de bots.
...
- **Enruta por prefijo de ruta** a pools de upstream balanceados (4 instancias por aplicación)
...
- **Caché de proxy** para activos estáticos de Juice Shop (zona de 10 MB, máximo de 100 MB, TTL de 60 s)
- **Registro de acceso deshabilitado** para prevenir el agotamiento de disco durante pruebas de carga CDN (logrotate como defensa en profundidad)
...
| `/health` | nginx | -- | -- | -- | Endpoint de salud en JSON (9 apps listadas) |
...
Esta es una pieza de un entorno de laboratorio más amplio. Cada componente es autónomo y se despliega de forma independiente:

- **Este componente** proporciona el servidor de origen (nginx + contenedores Docker en Azure VM)
- **CDN Simulator** proporciona la capa edge del CDN (caché nginx en Azure VM)
...
| **DVWA** | Estándar de la industria para pruebas WAF; niveles de seguridad ajustables (bajo/medio/alto/imposible); compilación personalizada php-fpm + nginx para rendimiento; backend compartido MariaDB 10.11 |
| **VAmPI** | Construida específicamente para el OWASP API Security Top 10; API REST con vulnerabilidades conocidas; gunicorn con 4 workers por instancia; ip_hash sticky para consistencia de SQLite |
...
| **whoami** | Servidor echo de solicitudes de Traefik; muestra los detalles completos de la solicitud tal como los ve el origen -- esencial para verificar la inyección de encabezados de F5 XC |
| **CSD Demo** | Página de checkout personalizada con 5 ataques estilo Magecart activables (card skimmer, formjacker, keylogger, cryptominer, DOM hijack); endpoint de exfiltración + panel del atacante; gunicorn single-worker para persistencia de estado en memoria |
| **DVGA** | Damn Vulnerable GraphQL Application; vulnerabilidades específicas de GraphQL incluyendo inyección, DoS, ataques de batching y bypass de autorización; UI GraphiQL para exploración interactiva; ip_hash sticky para SQLite por instancia |
| **RESTaurant** | Damn Vulnerable RESTaurant API Game; construida específicamente para el OWASP API Security Top 10 2023; FastAPI con Swagger UI; backend compartido PostgreSQL 15.4; cubre BOLA, BFLA, asignación masiva, SSRF e inyección |
| **crAPI** | OWASP Completely Ridiculous API; arquitectura de 7 microservicios que cubre BOLA, BFLA, asignación masiva, SSRF, manipulación de JWT e inyección NoSQL; puerto dedicado 8888 (SPA con rutas de API hardcodeadas); MailHog para captura de correo electrónico |
