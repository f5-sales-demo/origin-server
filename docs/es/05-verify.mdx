...
title: Verificación
description: >-
  Pruebas de humo con curl para los nueve endpoints de aplicación, verificación
  de estado de contenedores Docker, verificación de estado de nginx, inspección
  de logs de cloud-init y pasos comunes de resolución de problemas
sidebar:
  order: 5
i18n:
...
Ejecute estas verificaciones después de esperar 5--10 minutos para que cloud-init se complete.
...
Todos deberían devolver `200` (DVWA devuelve una redirección `302` al login).
...
Esto valida las 9 aplicaciones, endpoints de salud, registro/login de VAmPI, ciclo completo de exfiltración de CSD Demo, GraphQL de DVGA, Swagger de RESTaurant, crAPI en el puerto 8888, gzip de nginx y ocultación de versión.
...
Si los endpoints devuelven 502 o conexión rechazada, es posible que cloud-init aún esté aprovisionando:
...
El contenedor upstream no está listo o ha fallado:
...
Las imágenes Docker consumen aproximadamente 4.7 GiB. El registro de accesos está deshabilitado para prevenir el agotamiento del disco durante pruebas de carga con CDN. Logrotate está configurado como defensa en profundidad (límite de 500 MiB). Verifique el uso de disco:
...
Verifique si hay contenedores acercándose a sus límites de memoria (riesgo de OOM):
