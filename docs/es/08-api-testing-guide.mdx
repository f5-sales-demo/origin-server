...
Esta guía cataloga cada endpoint de API, vulnerabilidad intencional y payload de ataque en las tres aplicaciones de pruebas de seguridad de API. Utilícela para construir patrones de generación de tráfico para el desarrollo de perfiles de protección de API en F5 XC.
...
Todas las interacciones con DVGA utilizan un único endpoint (`/dvga/graphql`) con peticiones POST que contienen payloads JSON `{"query":"..."}`.
...
| `systemDiagnostics(cmd)` | Query | admin/password | Ejecutar comandos OS de lista blanca |
...
| `uploadPaste(filename, content)` | Mutation | No | Subir paste (recorrido de rutas) |

### Catálogo de Vulnerabilidades

#### 1. Denegación de Servicio (6 escenarios)

**Ataque de Consultas por Lote:**
```bash
...
**Consulta de Alto Consumo de Recursos (~82 segundos de respuesta):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**SSRF vía importPaste (sondear servicios internos):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Inyección de Logs (suplantación de nombre de operación):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Escritura Arbitraria de Archivos vía uploadPaste (recorrido de rutas):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Auth:** JWT vía POST form-encoded a `/restaurant/token`
**Roles:** Customer (predeterminado), Employee, Chef (admin)

### Configuración: Registro y Autenticación
...
| `/restaurant/menu` | GET | Sí | Cualquiera | Listar ítems del menú |
...
| `/restaurant/orders/{order_id}` | GET | Sí | Cualquiera | **BOLA** (acceder a órdenes de otros) |
...
**Fuerza bruta de secreto débil JWT (el secreto es de 6 dígitos: 97952):**
```bash
# Decodificar y falsificar tokens en jwt.io
# Secreto: 97952 (susceptible a fuerza bruta con hashcat -a 3 -m 16500 token '?d?d?d?d?d?d')
...
**Eliminar ítem del menú como Customer (debería requerir Employee+):**
```bash
...
# 4. Re-autenticar para obtener token de nivel Chef
TOKEN=$(curl -sf -X POST ${ORIGIN}/restaurant/token \
  -d "username=hacker&password=Hack123" | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
...
### Configuración: Registro, Verificación de Email, Inicio de Sesión
...
| `/identity/api/v2/user/change-email` | PUT | Sí | Cambio de email |
...
| Endpoint | Método | Auth | Vulnerabilidad |
|---|---|---|---|
| `/workshop/api/mechanic` | GET | Sí | **Exposición de datos** (emails de mecánicos) |
...
| `/workshop/api/shop/orders/{id}` | GET | No | **BOLA** (no requiere auth) |
...
```bash
# Primero obtener el UUID de su vehículo
...
#### Desafío 3 — Autenticación Rota: Fuerza Bruta de OTP de Restablecimiento de Contraseña
...
#### Desafío 4 — Exposición de Datos: Filtrar Emails de Mecánicos
...
#### Desafío 6 — DoS: Capa 7 vía Contact Mechanic
...
#### Desafíos 8 y 9 — Asignación Masiva: Ítems Gratuitos vía Manipulación de Órdenes
...
#### Desafío 11 — SSRF vía Contact Mechanic
...
# 1. Establecer header: {"alg":"none","typ":"JWT"} y codificar en base64url
# 2. Establecer payload: {"email":"admin@example.com","role":"admin"} y codificar en base64url
# 3. Unir con puntos, dejar la firma vacía: <header>.<payload>.
...
| **API2: Autenticación Rota** | Falsificación JWT (query `me`) | Secreto JWT débil (97952) | Fuerza bruta de OTP (v2), confusión de algoritmo JWT |
| **API3: BOPLA** | — | `PATCH /profile` (escalación de rol) | Video conversion_params, exposición de emails de mecánicos |
| **API4: Consumo de Recursos** | DoS por lote, recursión, alias, duplicación de campos | Enumeración de usuarios vía longitud de respuesta | DoS repeat_request de `contact_mechanic` |
...
### Fase 1: Línea Base (Flujo Normal)

Generar tráfico legítimo para establecer el comportamiento normal de la API antes de probar patrones de ataque.
...
**Pruebas de inyección:** Payloads SQL en parámetros de filtro/cupón, inyección en consultas GraphQL, inyección de comandos OS vía campos de parámetros.

**Pruebas de evasión de autenticación:** Falsificar tokens JWT, fuerza bruta de OTPs, usar versiones obsoletas de la API, confusión de algoritmo.

**Pruebas de SSRF:** URLs internas en los parámetros `importPaste`, `image_url` y `mechanic_api`.

**Pruebas de DoS:** Ataques GraphQL por lote/recursión/alias, `repeat_request_if_failed` con conteos altos.

### Fase 3: Cadenas de Ataque con Estado

Algunos ataques requieren secuencias de múltiples pasos con estado:

1. **Cadena de escalación de RESTaurant:** Registro → Token → PATCH de rol a Chef → Re-autenticación → Inyección de comandos
2. **Flujo completo de crAPI:** Registro → Verificar email (MailHog) → Inicio de sesión → Agregar vehículo → Contactar mecánico (SSRF) → Manipulación de órdenes
3. **De reconocimiento a explotación en DVGA:** Introspección → Descubrir systemDiagnostics → Fuerza bruta de credenciales de admin → Ejecución de comandos OS
...
| Sondeo SSRF | 5-20 | 2 min | Lento; cada uno dispara una solicitud HTTP del lado del servidor |
