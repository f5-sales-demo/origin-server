...
| `systemHealth` | Query | No | Verificación de salud |
...
| `uploadPaste(filename, content)` | Mutation | No | Subir paste (traversal de directorio) |
...
**Comando OS vía systemDiagnostics (requiere autenticación admin):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
#### 5. Bypass de Autorización (3 escenarios)
...
**Escritura Arbitraria de Archivos vía uploadPaste (traversal de directorio):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Roles:** Customer (predeterminado), Employee, Chef (administrador)
...
| `/restaurant/healthcheck` | GET | No | — | Verificación de salud |
...
| `/restaurant/orders/{order_id}` | GET | Sí | Cualquiera | **BOLA** (acceder a pedidos ajenos) |
...
**Acceder a pedidos de otros usuarios (enumerar offset):**
```bash
...
#### API3:2023 — Autorización Rota a Nivel de Propiedad de Objeto (BOPLA)
...
#### API5:2023 — Autorización Rota a Nivel de Función (BFLA)
...
# 4. Re-autenticar para obtener token con nivel Chef
TOKEN=$(curl -sf -X POST ${ORIGIN}/restaurant/token \
  -d "username=hacker&password=Hack123" | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
...
### Configuración: Registrar, Verificar Email, Iniciar Sesión
...
# 2. Verificar en MailHog el correo de verificación
...
#### Desafío 3 — Auth Rota: Fuerza Bruta del OTP de Restablecimiento de Contraseña
...
# Fuerza bruta del OTP de 4 dígitos (v2 NO tiene límite de tasa)
...
#### Desafíos 8 y 9 — Asignación Masiva: Artículos Gratis vía Manipulación de Pedidos
...
## Referencia Cruzada del OWASP API Security Top 10

| Categoría OWASP | DVGA | RESTaurant | crAPI |
|---|---|---|---|
| **API1: BOLA** | — | `PUT /profile`, `GET /orders/{id}` | Ubicación de vehículos, reportes de mecánicos, pedidos |
| **API2: Auth Rota** | Falsificación JWT (consulta `me`) | Secreto JWT débil (97952) | Fuerza bruta de OTP (v2), confusión de algoritmo JWT |
| **API3: BOPLA** | — | `PATCH /profile` (escalación de rol) | Video conversion_params, exposición de email de mecánicos |
...
| **API6: Asignación Masiva** | — | `PUT /orders` (status, quantity) | Status/quantity de pedidos, video conversion_params |
| **API7: SSRF** | Mutación `importPaste` | `PUT /menu` image_url | URL mechanic_api de `contact_mechanic` |
...
| **API9: Activos Inadecuados** | — | — | `/auth/v2/check-otp` deprecado (sin límite de tasa) |
...
**Pruebas de inyección:** Payloads SQL en parámetros filter/coupon, inyección de consultas GraphQL, inyección de comandos OS vía campos de parámetros.

**Pruebas de bypass de autenticación:** Falsificar tokens JWT, fuerza bruta de OTPs, usar versiones deprecadas de la API, confusión de algoritmo.

**Pruebas de SSRF:** URLs internas en parámetros `importPaste`, `image_url` y `mechanic_api`.

**Pruebas de DoS:** Ataques GraphQL de lote/recursión/alias, `repeat_request_if_failed` con conteos altos.

### Fase 3: Cadenas de Ataque con Estado

Algunos ataques requieren secuencias de múltiples pasos con estado:

1. **Cadena de escalación de RESTaurant:** Registrar → Token → PATCH rol a Chef → Re-autenticar → Inyección de comandos
2. **Flujo completo de crAPI:** Registro → Verificar email (MailHog) → Login → Agregar vehículo → Contactar mecánico (SSRF) → Manipulación de pedidos
3. **Reconocimiento a explotación en DVGA:** Introspección → Descubrir systemDiagnostics → Fuerza bruta de credenciales admin → Ejecución de comandos OS
...
| Sondeo SSRF | 5-20 | 2 min | Lento; cada una dispara una solicitud HTTP del lado del servidor |
