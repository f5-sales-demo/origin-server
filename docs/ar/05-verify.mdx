...
يجب أن تُرجع جميعها `200` (يُرجع DVWA `302` كإعادة توجيه إلى صفحة تسجيل الدخول).

### اختبار التحقق السريع التلقائي

يتضمن المستودع مجموعة اختبارات تحقق سريع من 39 نقطة:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

يقوم هذا بالتحقق من جميع التطبيقات التسعة، ونقاط نهاية الصحة، وتسجيل/تسجيل دخول VAmPI، ودورة تسريب البيانات الكاملة لـ CSD Demo، وGraphQL لـ DVGA، وSwagger لـ RESTaurant، وcrAPI على المنفذ 8888، وضغط gzip في nginx، وإخفاء الإصدار.
...
إذا أرجعت نقاط النهاية خطأ 502 أو رفض الاتصال، فقد يكون cloud-init لا يزال في مرحلة التهيئة:

```bash
ssh azureuser@${ORIGIN_IP} "cloud-init status"
```

- `status: running` -- لا يزال في مرحلة التهيئة، انتظر بضع دقائق
- `status: done` -- اكتملت التهيئة، تحقق من حاويات Docker
...
إذا لزم الأمر، أعد بناء وإعادة تشغيل جميع الخدمات:
