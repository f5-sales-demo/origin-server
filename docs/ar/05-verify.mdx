...
title: التحقق
description: >-
  اختبارات curl السريعة لجميع نقاط نهاية التطبيقات التسعة، وفحوصات صحة حاويات
...
## اختبارات الفحص السريع

قم بتشغيل هذه الفحوصات بعد الانتظار من 5 إلى 10 دقائق لاكتمال cloud-init.
...
يجب أن تُرجع جميعها `200` (يُرجع DVWA `302` إعادة توجيه إلى صفحة تسجيل الدخول).

### اختبار الفحص السريع التلقائي

يتضمن المستودع مجموعة اختبارات فحص سريع من 39 نقطة:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

يقوم هذا بالتحقق من جميع التطبيقات التسعة، ونقاط نهاية الصحة، وتسجيل/تسجيل الدخول في VAmPI، ودورة تسريب البيانات الكاملة في CSD Demo، وGraphQL في DVGA، وSwagger في RESTaurant، وcrAPI على المنفذ 8888، وضغط gzip في nginx، وإخفاء الإصدار.

### حالة حاويات Docker

قم بالاتصال عبر SSH بالجهاز الافتراضي وتحقق من أن جميع الحاويات الـ 41 تعمل:
...
### لا يزال cloud-init قيد التشغيل

إذا كانت نقاط النهاية تُرجع 502 أو رفض الاتصال، فقد يكون cloud-init لا يزال في مرحلة التجهيز:
...
### يُرجع nginx الخطأ 502

الحاوية الخلفية غير جاهزة أو تعطلت:
...
تستهلك صور Docker ما يقارب 4.7 جيبي بايت. تم تعطيل تسجيل الوصول لمنع استنفاد القرص تحت اختبار حمل CDN. تم تكوين logrotate كطبقة حماية إضافية (حد أقصى 500 ميبي بايت). تحقق من استخدام القرص:
...
تحقق من الحاويات التي تقترب من حدود الذاكرة المخصصة لها (خطر نفاد الذاكرة OOM):
