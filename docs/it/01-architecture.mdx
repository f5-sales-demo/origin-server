...
Questo componente fornisce un singolo server di origine che ospita molteplici applicazioni web vulnerabili per demo di test di sicurezza. Rappresenta l'"origin" in una tipica architettura di bilanciamento del carico -- il server backend dei contenuti che un load balancer HTTP F5 XC protegge.
...
- **Rimane in ascolto sulla porta 80** con `reuseport` e `backlog=4096` per traffico CDN ad alta concorrenza
- **Instrada per prefisso di percorso** verso pool upstream bilanciati (4 istanze per applicazione)
...
- **Proxy cache** per le risorse statiche di Juice Shop (zona 10 MB, massimo 100 MB, TTL 60 s)
...
- **Inoltra gli header del client** (`X-Real-IP`, `X-Forwarded-For`, `X-Forwarded-Proto`) per la visibilità sull'origine
...
| `/httpbin/` | httpbin_up | 4 | 8201-8204 | -- | Servizio richieste/risposte HTTP per demo API |
| `/whoami/` | whoami_up | 4 | 8082-8085 | -- | Diagnostica richieste -- mostra tutti gli header, IP del client |
...
Questo è un elemento di un ambiente lab più ampio. Ogni componente è autonomo e distribuito indipendentemente:
...
|---|---|
| **Juice Shop** | Progetto di punta OWASP; SPA moderna in Node.js con oltre 100 sfide che coprono la OWASP Top 10; mantenuta attivamente; 4 istanze con proxy cache |
| **DVWA** | Standard del settore per il test WAF; livelli di sicurezza regolabili (low/medium/high/impossible); build personalizzata php-fpm + nginx per le prestazioni; backend condiviso MariaDB 10.11 |
...
| **whoami** | Server echo delle richieste di Traefik; mostra tutti i dettagli della richiesta come li vede l'origine -- essenziale per verificare l'iniezione di header di F5 XC |
| **CSD Demo** | Pagina di checkout personalizzata con 5 attacchi Magecart attivabili (card skimmer, formjacker, keylogger, cryptominer, DOM hijack); endpoint di esfiltrazione + dashboard dell'attaccante; gunicorn single-worker per la persistenza dello stato in memoria |
| **DVGA** | Damn Vulnerable GraphQL Application; vulnerabilità specifiche di GraphQL tra cui injection, DoS, attacchi di batching e bypass dell'autorizzazione; UI GraphiQL per l'esplorazione interattiva; ip_hash sticky per SQLite per istanza |
| **RESTaurant** | Damn Vulnerable RESTaurant API Game; costruita appositamente per OWASP API Security Top 10 2023; FastAPI con Swagger UI; backend condiviso PostgreSQL 15.4; copre BOLA, BFLA, mass assignment, SSRF e injection |
