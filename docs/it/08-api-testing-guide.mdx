...
Questa guida cataloga ogni endpoint API, vulnerabilità intenzionale e payload di attacco nelle tre applicazioni di test della sicurezza API. Utilizzatela per costruire pattern di generazione del traffico per lo sviluppo di profili di protezione API F5 XC.
...
Tutte le interazioni con DVGA utilizzano un singolo endpoint (`/dvga/graphql`) con richieste POST contenenti payload JSON `{"query":"..."}`.
...
|---|---|---|---|
| `pastes(public, filter, limit)` | Query | No | Elenco paste (SQL injection via filter) |
| `paste(id)` | Query | No | Recupero singola paste |
| `me(token)` | Query | No | Recupero utente tramite JWT (vulnerabile a falsificazione) |
| `systemHealth` | Query | No | Controllo stato |
| `systemUpdate` | Query | No | Query lenta (~82s, vettore DoS) |
| `systemDiagnostics(cmd)` | Query | admin/password | Esecuzione comandi OS in whitelist |
| `createPaste(title, content, public)` | Mutation | No | Creazione paste (XSS via content) |
| `importPaste(host, port, path, scheme)` | Mutation | No | Importazione paste remota (SSRF, command injection) |
...
**Query ad Alta Intensità di Risorse (risposta ~82 secondi):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Duplicazione dei Campi (ripetizione campo 500+ volte):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Auth:** JWT tramite POST form-encoded a `/restaurant/token`
...
# Ottenimento del token JWT (NOTA: form-encoded, non JSON)
...
| `/restaurant/healthcheck` | GET | No | — | Controllo stato |
...
| `/restaurant/menu` | PUT | Sì | Employee+ | Creazione menu (**SSRF** tramite immagine) |
| `/restaurant/menu/{item_id}` | PUT | Sì | Employee+ | Aggiornamento menu (**SSRF** tramite immagine) |
...
| `/restaurant/orders` | POST | Sì | Qualsiasi | Creazione ordine |
...
| `/restaurant/reset-password/new-password` | POST | No | — | Impostazione nuova password |
| `/restaurant/referral-code` | GET | Sì | Qualsiasi | Ottenimento codice referral |
| `/restaurant/apply-referral` | POST | Sì | Qualsiasi | Applicazione referral |
...
**Sondaggio di endpoint interni tramite image_url del menu (richiede ruolo Employee):**
```bash
# Prima effettuare l'escalation a Employee tramite BOPLA, poi:
...
**Auth:** Token JWT Bearer (RS256, vulnerabile a confusione dell'algoritmo)
...
# 2. Controllare MailHog per l'email di verifica
# Navigare a ${CRAPI}/mailhog/ o utilizzare l'API di MailHog:
...
| `/identity/api/v2/user/videos` | POST | Sì | Upload video |
...
| `/community/api/v2/community/posts` | POST | Sì | Creazione post blog |
| `/community/api/v2/community/posts/{id}/comments` | POST | Sì | Aggiunta commento |
...
| Endpoint | Metodo | Auth | Vulnerabilità |
|---|---|---|---|
| `/workshop/api/mechanic` | GET | Sì | **Esposizione dati** (email dei meccanici) |
...
| `/workshop/api/shop/orders/` | POST | Sì | Creazione ordine |
...
```bash
# Prima ottenere l'UUID del proprio veicolo
curl -sf ${CRAPI}/identity/api/v2/vehicle/vehicles \
  -H "Authorization: Bearer ${TOKEN}"

# Accesso al veicolo di un altro utente (sostituire UUID)
...
#### Sfida 3 — Broken Auth: Brute-Force dell'OTP per il Reset Password

```bash
# Richiesta OTP per la vittima
...
#### Sfida 4 — Esposizione Dati: Fuga di Email dei Meccanici
...
```bash
# Caricare un video, poi ispezionare la risposta
curl -sf ${CRAPI}/identity/api/v2/user/videos \
  -H "Authorization: Bearer ${TOKEN}" | python3 -m json.tool
# Cercare il campo conversion_params nella risposta
```
...
```bash
# Un utente regolare può accedere all'endpoint admin
...
```bash
# Cambiare GET in PUT, modificare status e quantity
...
```bash
# Falsificazione di un JWT con algoritmo "none" (nessuna verifica della firma):
# 1. Impostare header: {"alg":"none","typ":"JWT"} e codificarlo in base64url
# 2. Impostare payload: {"email":"admin@example.com","role":"admin"} e codificarlo in base64url
# 3. Unire con punti, lasciare la firma vuota: <header>.<payload>.
...
| **API2: Broken Auth** | Falsificazione JWT (query `me`) | Segreto JWT debole (97952) | Brute-force OTP (v2), confusione algoritmo JWT |
| **API3: BOPLA** | — | `PATCH /profile` (escalation del ruolo) | Video conversion_params, esposizione email meccanici |
| **API4: Resource Consumption** | Batch, ricorsione, alias, duplicazione campi DoS | Enumerazione utenti tramite lunghezza risposta | `contact_mechanic` repeat_request DoS |
| **API5: BFLA** | — | `DELETE /menu`, `PUT /users/update_role` | `DELETE /admin/videos` come utente regolare |
| **API6: Mass Assignment** | — | `PUT /orders` (status, quantity) | Status/quantity ordini, video conversion_params |
...
| **API9: Improper Assets** | — | — | `/auth/v2/check-otp` deprecato (nessun rate limit) |
| **API10: Unsafe Consumption** | — | — | (indiretto tramite SSRF) |
...
Generare traffico legittimo per stabilire il comportamento normale dell'API prima di testare i pattern di attacco.
...
**Test BOLA:** Enumerare ID sequenziali, scambiare identificatori utente nelle richieste, accedere a risorse senza possesso.
...
**Test bypass dell'autenticazione:** Falsificazione token JWT, brute-force OTP, utilizzo di versioni API deprecate, confusione dell'algoritmo.
...
2. **Flusso completo crAPI:** Registrazione → Verifica email (MailHog) → Login → Aggiunta veicolo → Contatto meccanico (SSRF) → Manipolazione ordini
3. **Dalla ricognizione all'exploit DVGA:** Introspezione → Scoperta systemDiagnostics → Brute-force credenziali admin → Esecuzione comandi OS

### Raccomandazioni sulla Frequenza

| Pattern | Richieste/sec | Durata | Note |
|---|---|---|---|
| Baseline (per app) | 10-50 | 5 min | Stabilire impronta del traffico normale |
| Enumerazione BOLA | 100-500 | 2 min | Scansione ID sequenziali |
| Brute-force OTP | 1000+ | Fino al ritrovamento | 10.000 tentativi massimi (4 cifre) |
| DoS GraphQL | 10-50 | 30 sec | Ogni richiesta è costosa lato server |
| Fuzzing injection | 50-200 | 5 min | Variare i payload per ogni richiesta |
