...
description: >-
  ...
  log di cloud-init e procedure comuni di risoluzione dei problemi
sidebar:
  order: 5
i18n:
...
Questo valida tutte e 9 le applicazioni, gli endpoint di integrità, la registrazione/login di VAmPI, il ciclo completo di esfiltrazione di CSD Demo, GraphQL di DVGA, Swagger di RESTaurant, crAPI sulla porta 8888, la compressione gzip di nginx e l'occultamento della versione.

### Stato dei container Docker

Accedere tramite SSH alla VM e verificare che tutti i 41 container siano in esecuzione:
...
Le immagini Docker occupano circa 4,7 GiB. La registrazione dei log di accesso è disabilitata per evitare l'esaurimento dello spazio su disco durante i test di carico CDN. Logrotate è configurato come difesa in profondità (limite di 500 MiB). Verificare l'utilizzo del disco:
...
Verificare la presenza di container che si avvicinano ai limiti di memoria (rischio OOM):
