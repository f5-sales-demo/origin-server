...
title: Verifica
description: >-
  Test di verifica curl per tutti e nove gli endpoint applicativi, controlli di
  integrità dei container Docker, verifica dello stato di nginx, ispezione dei
  log di cloud-init e procedure di risoluzione dei problemi comuni
sidebar:
  order: 5
i18n:
...
Il repository include una suite di test di verifica con 39 controlli:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

Questo valida tutte le 9 applicazioni, gli endpoint di integrità, la registrazione/login di VAmPI, il ciclo completo di esfiltrazione di CSD Demo, GraphQL di DVGA, Swagger di RESTaurant, crAPI sulla porta 8888, la compressione gzip di nginx e l'occultamento della versione.

### Stato dei container Docker

Accedere alla VM tramite SSH e verificare che tutti i 41 container siano in esecuzione:
...
- `status: running` -- ancora in fase di provisioning, attendere qualche minuto
- `status: done` -- provisioning completato, verificare i container Docker
- `status: error` -- consultare i log di seguito
...
# Riavviare tutte le istanze di una specifica app
...
Le immagini Docker occupano circa 4,7 GiB. La registrazione degli accessi è disabilitata per prevenire l'esaurimento del disco durante i test di carico CDN. Logrotate è configurato come difesa in profondità (limite di 500 MiB). Verificare l'utilizzo del disco:
...
Verificare la presenza di container che si avvicinano ai propri limiti di memoria (rischio OOM):
