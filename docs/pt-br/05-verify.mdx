...
title: Verificar
description: >-
  Testes de verificação curl para todos os nove endpoints de aplicação,
  verificações de integridade de contêineres Docker, verificação de status do
  nginx, inspeção de logs do cloud-init e etapas comuns de solução de problemas
sidebar:
  order: 5
i18n:
...
Execute essas verificações após aguardar 5 a 10 minutos para a conclusão do cloud-init.
...
Todos devem retornar `200` (DVWA retorna redirecionamento `302` para login).

### Teste de Verificação Automatizado

O repositório inclui uma suíte de testes de verificação com 39 pontos:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

Isso valida todas as 9 aplicações, endpoints de integridade, registro/login do VAmPI, ciclo completo de exfiltração do CSD Demo, GraphQL do DVGA, Swagger do RESTaurant, crAPI na porta 8888, gzip do nginx e ocultação de versão.

### Status dos Contêineres Docker

Conecte-se via SSH à VM e verifique se todos os 41 contêineres estão em execução:
...
Se os endpoints retornarem 502 ou conexão recusada, o cloud-init ainda pode estar provisionando:
...
```bash
# Verifique os logs do contêiner (cada aplicação tem 4 instâncias: -1 a -4)
...
As imagens Docker consomem aproximadamente 4,7 GiB. O registro de acesso está desabilitado para evitar esgotamento de disco durante testes de carga com CDN. O logrotate está configurado como defesa em profundidade (limite de 500 MiB). Verifique o uso de disco:
