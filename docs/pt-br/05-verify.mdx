...
title: Verificar
description: >-
  testes de smoke com curl para todos os nove endpoints da aplicação,
  verificações de saúde de contêineres Docker, verificação do status do nginx,
...
Execute estas verificações após aguardar 5--10 minutos para o cloud-init ser concluído.
...
Todos devem retornar `200` (DVWA retorna redirecionamento `302` para o login).

### Teste de Smoke Automatizado

O repositório inclui um conjunto de 39 pontos de teste de smoke:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

Este valida todas as 9 aplicações, endpoints de saúde, registro/login do VAmPI, round-trip de exfiltração do CSD Demo, GraphQL do DVGA, Swagger do RESTaurant, crAPI na porta 8888, gzip do nginx e ocultação de versão.
...
### Contêiner não inicializando

```bash
# Verificar logs do contêiner (cada aplicação tem 4 instâncias: -1 a -4)
...
O contêiner upstream não está pronto ou travou:

```bash
# Verificar log de erros do nginx
ssh azureuser@${ORIGIN_IP} "sudo tail -20 /var/log/nginx/error.log"

# Reiniciar todas as instâncias de uma aplicação específica
ssh azureuser@${ORIGIN_IP} "for i in 1 2 3 4; do sudo docker restart juice-shop-\$i; done"

# Reconstruir e reiniciar todos os contêineres
ssh azureuser@${ORIGIN_IP} "cd /opt/origin-server && sudo docker compose build && sudo docker compose up -d"
```

### Espaço em disco

As imagens Docker consomem aproximadamente 4,7 GiB. O registro de acesso está desativado para evitar esgotamento do disco durante testes de carga de CDN. O Logrotate está configurado como defesa em profundidade (limite de 500 MiB). Verifique o uso do disco:
...
Verifique contêineres que se aproximam dos limites de memória (risco de OOM):
