...
title: Verificação
description: >-
  curl smoke tests para todos os nove endpoints de aplicação, verificações de
  saúde de contêineres Docker, verificação de status do nginx, inspeção de logs
  do cloud-init e etapas comuns de solução de problemas
sidebar:
  order: 5
i18n:
...
Execute essas verificações após aguardar de 5 a 10 minutos para a conclusão do cloud-init.
...
Todos devem retornar `200` (DVWA retorna `302` redirecionamento para login).

### Smoke Test Automatizado

O repositório inclui uma suíte de smoke test com 39 pontos de verificação:
...
### Contêiner não está iniciando

```bash
# Verifique os logs do contêiner (cada aplicação possui 4 instâncias: -1 até -4)
...
```bash
# Verifique o log de erros do nginx
ssh azureuser@${ORIGIN_IP} "sudo tail -20 /var/log/nginx/error.log"

# Reinicie todas as instâncias de uma aplicação específica
ssh azureuser@${ORIGIN_IP} "for i in 1 2 3 4; do sudo docker restart juice-shop-\$i; done"

# Reconstrua e reinicie todos os contêineres
ssh azureuser@${ORIGIN_IP} "cd /opt/origin-server && sudo docker compose build && sudo docker compose up -d"
```

### Espaço em disco

As imagens Docker consomem aproximadamente 4,7 GiB. O registro de acesso está desabilitado para evitar esgotamento do disco durante testes de carga CDN. O logrotate está configurado como defesa em profundidade (limite de 500 MiB). Verifique o uso do disco:
