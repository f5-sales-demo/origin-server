---
title: API 测试指南
description: 全面的端点目录、漏洞映射和攻击载荷，涵盖 DVGA、RESTaurant 和 crAPI — 用于流量生成和 F5 XC API 防护配置文件开发的入门参考
sidebar:
  order: 8
i18n:
...
本指南汇总了三个 API 安全测试应用中的所有 API 端点、已知漏洞和攻击载荷。利用它来构建流量生成模式，以支持 F5 XC API 防护配置文件的开发。
...
所有 DVGA 交互都使用单一端点 (`/dvga/graphql`)，通过 POST 请求发送包含 JSON `{"query":"..."}` 的载荷。
...
| 操作 | 类型 | 认证 | 用途 |
|---|---|---|---|
| `pastes(public, filter, limit)` | Query | 否 | 列出粘贴内容（filter 参数存在 SQL 注入） |
| `paste(id)` | Query | 否 | 获取单个粘贴内容 |
| `me(token)` | Query | 否 | 通过 JWT 获取用户（存在伪造漏洞） |
| `systemHealth` | Query | 否 | 健康检查 |
| `systemUpdate` | Query | 否 | 慢查询（约82秒，DoS 攻击向量） |
| `systemDiagnostics(cmd)` | Query | admin/password | 执行白名单中的操作系统命令 |
| `createPaste(title, content, public)` | Mutation | 否 | 创建粘贴内容（content 参数存在 XSS） |
| `importPaste(host, port, path, scheme)` | Mutation | 否 | 导入远程粘贴内容（SSRF、命令注入） |
| `uploadPaste(filename, content)` | Mutation | 否 | 上传粘贴内容（路径遍历） |
...
**认证：** 通过表单编码 POST 请求到 `/restaurant/token` 获取 JWT
...
| `/restaurant/menu` | PUT | 是 | Employee+ | 创建菜单（通过 image 实现 **SSRF**） |
| `/restaurant/menu/{item_id}` | PUT | 是 | Employee+ | 更新菜单（通过 image 实现 **SSRF**） |
| `/restaurant/menu/{item_id}` | DELETE | 是 | 任意 | **BFLA**（任何用户都可删除） |
...
**JWT 弱密钥暴力破解（密钥为6位数字：97952）：**
```bash
# 在 jwt.io 上解码和伪造令牌
# 密钥: 97952（可通过 hashcat -a 3 -m 16500 token '?d?d?d?d?d?d' 暴力破解）
...
**以 Customer 身份删除菜单项（应需要 Employee+ 权限）：**
```bash
...
# 2. 在 MailHog 中查看验证邮件
# 浏览 ${CRAPI}/mailhog/ 或使用 MailHog API：
...
| `/identity/api/auth/v2/check-otp` | POST | 否 | **无速率限制**（暴力破解4位 OTP） |
...
| `/identity/api/v2/user/videos/{id}` | PUT | 是 | **批量分配**（命令注入） |
| `/identity/api/v2/admin/videos/{id}` | DELETE | 是 | **BFLA**（未检查管理员权限） |
...
| 端点 | 方法 | 认证 | 漏洞 |
|---|---|---|---|
| `/workshop/api/mechanic` | GET | 是 | **数据暴露**（技工邮箱） |
...
| `/workshop/api/shop/orders/{id}` | PUT | 是 | **批量分配**（status、quantity） |
...
curl -sf ${CRAPI}/identity/api/v2/user/videos \
  -H "Authorization: Bearer ${TOKEN}" | python3 -m json.tool
# 查看响应中的 conversion_params 字段
```

#### 挑战 6 — DoS：通过联系技工实现第7层攻击
...
#### 挑战 8 和 9 — 批量分配：通过订单操纵获取免费商品

```bash
# 将 GET 更改为 PUT，修改 status 和 quantity
...
|---|---|---|---|
| **API1: BOLA** | — | `PUT /profile`、`GET /orders/{id}` | 车辆位置、技工报告、订单 |
| **API2: 认证失效** | JWT 伪造（`me` 查询） | JWT 弱密钥 (97952) | OTP 暴力破解 (v2)、JWT 算法混淆 |
| **API3: BOPLA** | — | `PATCH /profile`（角色提升） | 视频 conversion_params、技工邮箱暴露 |
| **API4: 资源消耗** | 批量、递归、别名、字段重复 DoS | 通过响应长度进行用户枚举 | `contact_mechanic` repeat_request DoS |
| **API5: BFLA** | — | `DELETE /menu`、`PUT /users/update_role` | 普通用户执行 `DELETE /admin/videos` |
| **API6: 批量分配** | — | `PUT /orders`（status、quantity） | 订单 status/quantity、视频 conversion_params |
...
| **API10: 不安全的消费** | — | — | （通过 SSRF 间接利用） |
| **GraphQL 特有** | 自省查询、批量操作、递归、别名、字段重复、循环片段 | — | — |
...
### 阶段 2：按 OWASP 类别分类的攻击流量

**BOLA 测试：** 枚举顺序 ID，在请求中替换用户标识符，在没有所有权的情况下访问资源。
...
1. **RESTaurant 提权链：** 注册 → 获取令牌 → PATCH 角色至 Chef → 重新认证 → 命令注入
2. **crAPI 完整流程：** 注册 → 验证邮箱（MailHog）→ 登录 → 添加车辆 → 联系技工（SSRF）→ 订单操纵
3. **DVGA 从侦察到利用：** 自省查询 → 发现 systemDiagnostics → 暴力破解管理员凭据 → 操作系统命令执行
...
| OTP 暴力破解 | 1000+ | 直到找到为止 | 最多 10,000 次尝试（4位数字） |
| GraphQL DoS | 10-50 | 30 秒 | 每个请求在服务端开销较大 |
| 注入模糊测试 | 50-200 | 5 分钟 | 每个请求变换载荷 |
| SSRF 探测 | 5-20 | 2 分钟 | 较慢；每个请求触发服务端 HTTP 请求 |
