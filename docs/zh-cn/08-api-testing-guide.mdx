---
title: API 测试指南
description: 全面的端点目录、漏洞映射和攻击载荷，涵盖 DVGA、RESTaurant 和 crAPI — 用于流量生成和 F5 XC API 防护策略开发的入门参考
sidebar:
  order: 8
i18n:
...
本指南编目了三个 API 安全测试应用中的所有 API 端点、预设漏洞和攻击载荷。使用它来构建用于 F5 XC API 防护策略开发的流量生成模式。
...
所有 DVGA 交互都使用单一端点 (`/dvga/graphql`)，通过 POST 请求发送包含 JSON `{"query":"..."}` 载荷。
...
**通过 systemDiagnostics 的 OS 命令（需要管理员认证）：**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
| 端点 | 方法 | 认证 | 角色 | 漏洞 |
|---|---|---|---|---|
| `/restaurant/register` | POST | 否 | — | 用户注册 |
...
| `/restaurant/profile` | PATCH | 是 | 任意 | **BOPLA**（批量赋值角色） |
| `/restaurant/users/update_role` | PUT | 是 | 任意 | **BFLA**（角色提升） |
| `/restaurant/menu` | GET | 是 | 任意 | 列出菜单项 |
| `/restaurant/menu` | PUT | 是 | Employee+ | 创建菜单（通过 image 的 **SSRF**） |
| `/restaurant/menu/{item_id}` | PUT | 是 | Employee+ | 更新菜单（通过 image 的 **SSRF**） |
| `/restaurant/menu/{item_id}` | DELETE | 是 | 任意 | **BFLA**（任何用户均可删除） |
| `/restaurant/orders` | GET | 是 | 任意 | **BOLA**（查看所有订单） |
| `/restaurant/orders` | POST | 是 | 任意 | 创建订单 |
| `/restaurant/orders/{order_id}` | GET | 是 | 任意 | **BOLA**（访问其他用户订单） |
...
| `/restaurant/apply-referral` | POST | 是 | 任意 | 使用推荐码 |
...
**Web UI：** `${CRAPI}/`
**MailHog：** `${CRAPI}/mailhog/`（邮件捕获用于验证）
...
| `/identity/api/v2/vehicle/{uuid}/location` | GET | 是 | **BOLA**（任意用户的车辆） |
...
| **API4: 资源消耗** | 批量、递归、别名、字段重复 DoS | 通过响应长度的用户枚举 | `contact_mechanic` repeat_request DoS |
| **API5: BFLA** | — | `DELETE /menu`、`PUT /users/update_role` | 普通用户 `DELETE /admin/videos` |
...
| **GraphQL 特有** | 自省、批量处理、递归、别名、字段重复、循环片段 | — | — |
...
### 阶段 2：按 OWASP 类别的攻击流量

**BOLA 测试：** 枚举顺序 ID，交换请求中的用户标识符，在没有所有权的情况下访问资源。

**注入测试：** 在 filter/coupon 参数中使用 SQL 载荷，GraphQL 查询注入，通过参数字段的 OS 命令注入。

**认证绕过测试：** 伪造 JWT 令牌，暴力破解 OTP，使用已弃用的 API 版本，算法混淆。

**SSRF 测试：** 在 `importPaste`、`image_url` 和 `mechanic_api` 参数中使用内部 URL。

**DoS 测试：** GraphQL 批量/递归/别名攻击，使用高次数的 `repeat_request_if_failed`。
...
3. **DVGA 侦察到利用：** 自省 → 发现 systemDiagnostics → 暴力破解管理员凭据 → OS 命令执行
...
| OTP 暴力破解 | 1000+ | 直到找到 | 最多 10,000 次尝试（4 位数字） |
| GraphQL DoS | 10-50 | 30 秒 | 每个请求在服务端开销很大 |
| 注入模糊测试 | 50-200 | 5 分钟 | 每个请求使用不同的载荷 |
| SSRF 探测 | 5-20 | 2 分钟 | 速度较慢；每次触发服务端 HTTP 请求 |
