---
title: 验证
description: curl 冒烟测试覆盖全部九个应用端点、Docker 容器健康检查、nginx 状态验证、cloud-init 日志检查以及常见故障排查步骤
sidebar:
  order: 5
i18n:
...
仓库中包含一个 39 项检查的冒烟测试套件：

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

该测试验证全部 9 个应用、健康检查端点、VAmPI 注册/登录、CSD Demo 数据泄露往返测试、DVGA GraphQL、RESTaurant Swagger、crAPI 8888 端口、nginx gzip 以及版本隐藏。

### Docker 容器状态

通过 SSH 登录虚拟机，验证全部 41 个容器是否正在运行：
...
Docker 镜像大约占用 4.7 GiB。为防止在 CDN 负载测试期间磁盘耗尽，访问日志已被禁用。Logrotate 已配置为纵深防御措施（上限 500 MiB）。验证磁盘使用情况：
