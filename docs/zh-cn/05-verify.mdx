...
该测试验证所有 9 个应用、健康检查端点、VAmPI 注册/登录、CSD Demo 数据外泄往返测试、DVGA GraphQL、RESTaurant Swagger、crAPI 8888 端口、nginx gzip 以及版本隐藏。

### Docker 容器状态

通过 SSH 连接到虚拟机，验证所有 41 个容器正在运行：
...
- `status: done` -- 配置已完成，检查 Docker 容器
- `status: error` -- 检查以下日志
...
Docker 镜像大约占用 4.7 GiB。访问日志已禁用，以防止在 CDN 负载测试期间磁盘空间耗尽。Logrotate 已配置为纵深防御措施（500 MiB 上限）。验证磁盘使用情况：
