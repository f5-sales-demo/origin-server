...
description: >-
  ...
  reuseport、粘性会话和代理缓存，路由到 Juice Shop、DVWA-FPM、VAmPI、httpbin、whoami、CSD
...
此组件提供一个托管多个漏洞 Web 应用程序的单一源服务器，用于安全测试演示。它代表典型负载均衡器架构中的"源站"——即 F5 XC HTTP 负载均衡器所保护的后端内容服务器。
...
此组件使用一个专门构建的虚拟机替代真实的生产应用服务器，运行众所周知的漏洞应用程序来触发 WAF 规则、API 安全策略和 Bot 检测。
...
- **代理缓存**用于 Juice Shop 静态资源（10 MB 缓存区，100 MB 最大容量，60 秒 TTL）
- **禁用访问日志**以防止 CDN 负载测试下磁盘耗尽（logrotate 作为纵深防御措施）
- **传递客户端头**（`X-Real-IP`、`X-Forwarded-For`、`X-Forwarded-Proto`）以确保源站可见性
...
| `/dvwa/` | dvwa | 4 + MariaDB | 8101-8104 | `hash $cookie_PHPSESSID` | 经典 WAF 测试，可调节难度级别 |
...
操作人员逐个添加组件。每个组件的文档编写方式使 AI 助手能够读取并自主部署基础设施。

## 选择这些应用的原因

| 应用 | 选择原因 |
|---|---|
| **Juice Shop** | OWASP 旗舰项目；现代 Node.js SPA，包含 100+ 个覆盖 OWASP Top 10 的挑战；持续维护；4 个实例配合代理缓存 |
| **DVWA** | WAF 测试的行业标准；可调节安全级别（低/中/高/不可能）；自定义 php-fpm + nginx 构建以提升性能；共享 MariaDB 10.11 后端 |
| **VAmPI** | 专为 OWASP API 安全 Top 10 构建；包含已知漏洞的 REST API；每个实例配置 gunicorn 4 个 worker；ip_hash 粘性确保 SQLite 一致性 |
| **httpbin** | Kenneth Reitz 的经典 HTTP 测试服务；gunicorn 配置 4 个 gevent worker；适用于 API 演示和请求检查 |
| **whoami** | Traefik 的请求回显服务器；显示源站看到的完整请求详情——对于验证 F5 XC 头注入至关重要 |
| **CSD Demo** | 自定义结账页面，包含 5 种可切换的 Magecart 风格攻击（银行卡窃取器、表单劫持、键盘记录器、加密挖矿、DOM 劫持）；数据窃取端点 + 攻击者仪表板；gunicorn 单 worker 以保持内存状态持久化 |
| **DVGA** | Damn Vulnerable GraphQL Application；GraphQL 特定漏洞，包括注入、DoS、批量攻击和授权绕过；GraphiQL UI 支持交互式探索；ip_hash 粘性确保每实例 SQLite 一致性 |
| **RESTaurant** | Damn Vulnerable RESTaurant API Game；专为 OWASP API 安全 Top 10 2023 构建；FastAPI 配合 Swagger UI；共享 PostgreSQL 15.4 后端；涵盖 BOLA、BFLA、批量赋值、SSRF 和注入 |
