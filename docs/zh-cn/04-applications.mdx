...
每个应用程序以 4 个负载均衡的 Docker 容器运行在 nginx 上游池后面，通过虚拟机上的 nginx 反向代理进行访问。粘性会话确保有状态应用程序（Juice Shop、DVWA、VAmPI、CSD Demo）的请求路由保持一致。所有应用程序都是故意设计为存在漏洞的，专门用于安全测试。

在以下所有示例中，请将 `<ORIGIN>` 替换为 `http://<PUBLIC_IP>`。通过 Terraform 部署后，使用 `terraform output -raw public_ip` 获取 IP。
...
OWASP Juice Shop 是最现代且维护最活跃的漏洞 Web 应用程序。它在一个真实的电商应用中涵盖了整个 OWASP Top 10，包含 100 多个挑战。
...
| 基于 DOM 的 XSS | CSD | 在 URL 片段中放置载荷 |
| 身份认证缺陷 | WAF | 暴力破解登录、JWT 操纵 |
| API 滥用 | API 安全 | 未授权访问 `/api/` 端点 |
| 敏感数据泄露 | API 安全 | 未经授权访问用户数据 |
| CSRF | WAF | 对个人资料更改进行跨站请求伪造 |
...
| 文件包含 | WAF | Low：直接路径遍历，High：已净化 |
| XSS（反射型） | WAF、CSD | Low：基本的 `<script>`，High：编码绕过 |
| XSS（存储型） | WAF、CSD | Low：留言板中的持久化脚本 |
...
- **Low** -- 无输入验证。所有攻击都能成功。适合 WAF 签名演示。
- **Medium** -- 基本过滤。部分攻击需要编码或绕过技术。
- **High** -- 强过滤。只有高级技术才能成功。适合展示 WAF 的局限性。
- **Impossible** -- 完全安全的实现。展示正确的防御编码方式。

### 初始设置

DVWA 在首次部署后需要一次性的数据库设置：
...
| **实例** | 4 个（端口 5101-5104），通过 `ip_hash` 实现粘性会话（每个实例独立 SQLite） |
| **资源** | 每个实例 0.5 CPU / 512 MiB RAM |
| **框架** | Python / Flask / gunicorn |
| **项目** | [github.com/erev0s/VAmPI](https://github.com/erev0s/VAmPI) |

VAmPI 是专门为测试 OWASP API 安全 Top 10 而构建的。它提供了一个包含故意漏洞的真实 REST API。每个实例运行带有 4 个 worker 的 gunicorn 和独立的 SQLite 数据库。`ip_hash` 粘性会话确保来自同一客户端 IP 的注册和登录请求始终命中同一个实例。

### 演示场景

| 场景 | OWASP API Top 10 | 方法 |
|---|---|---|
| 对象级授权失效（BOLA） | API1 | 通过操纵 ID 访问其他用户的数据 |
| 身份认证失效 | API2 | 弱令牌处理，无速率限制 |
| 过度数据暴露 | API3 | API 返回的数据超出客户端所需 |
...
|---|---|
| **路径** | `/httpbin/` |
| **镜像** | `kennethreitz/httpbin:latest`，自定义 gunicorn CMD（`-w 4 -k gevent --timeout 30`） |
| **实例** | 4 个（端口 8201-8204），轮询（无状态） |
| **资源** | 每个实例 0.5 CPU / 256 MiB RAM |
| **框架** | Python / Flask / gunicorn + gevent |
| **项目** | [httpbin.org](https://httpbin.org) |

httpbin 是一个简单的 HTTP 请求/响应服务，适用于基本的 API 演示、测试请求头和验证代理行为。

### 常用端点

| 端点 | 用途 |
|---|---|
| `/httpbin/get` | 返回 GET 请求数据（请求头、参数、来源） |
...
whoami 是 Traefik 的轻量级请求回显服务器。它显示源服务器所接收到的 HTTP 请求的每个细节——主机名、IP 地址、所有请求头、方法和 URL。这是验证 F5 XC 是否注入了正确请求头时最重要的诊断工具。
...
| 验证 Bot 防护标记 | `X-Volterra-Bot-Type`、`X-Volterra-Bot-Verified` 请求头 |
...
| **实例** | 4 个（端口 5201-5204），通过 `ip_hash` 实现粘性会话（每个实例独立 SQLite） |
| **资源** | 每个实例 0.5 CPU / 256 MiB RAM |
...
RESTaurant 是一个游戏化的漏洞 REST API，涵盖 OWASP API 安全 Top 10 2023。它使用 FastAPI，在 `/restaurant/docs` 提供自动生成的 Swagger UI 文档。所有 4 个实例共享单个 PostgreSQL 数据库，因此轮询负载均衡无需粘性会话即可正常工作。

### 演示场景

| 场景 | OWASP API Top 10 2023 | 方法 |
|---|---|---|
| 对象级授权失效（BOLA） | API1 | 通过操纵 ID 访问其他用户的订单 |
| 身份认证失效 | API2 | 弱令牌处理、凭据填充 |
| 对象属性级授权失效 | API3 | 对用户资料字段进行批量赋值 |
| 不受限的资源消耗 | API4 | 端点无速率限制 |
| 功能级授权失效（BFLA） | API5 | 以普通用户身份访问管理员端点 |
| 服务端请求伪造（SSRF） | API7 | 操纵服务端 URL 请求 |
...
crAPI 是 OWASP 旗舰级 API 安全测试项目。它作为 7 个微服务运行在专用端口（8888）上，因为 React SPA 硬编码了其 API 路径，无法在路径前缀后面提供服务。NSG 允许端口 8888 的入站流量。

MailHog 捕获 crAPI 发送的所有电子邮件（账户验证、密码重置）。通过 SSH 隧道在端口 18025 访问 MailHog。

### 挑战类别

| 类别 | 漏洞 |
|---|---|
| BOLA（对象级授权失效） | 访问其他用户的车辆、订单和报告 |
| BFLA（功能级授权失效） | 提升为管理员、访问受限端点 |
| 批量赋值 | 通过 API 修改受保护字段（角色、余额） |
| SSRF（服务端请求伪造） | 操纵服务端 URL 获取 |
| JWT 操纵 | 伪造或修改 JWT 令牌以进行权限提升 |
...
| Bot 防护 -- 暴力破解 | DVWA | Juice Shop |
| Bot 防护 -- 爬取 | Juice Shop | -- |
| 客户端防护 -- DOM XSS | Juice Shop | -- |
| 客户端防护 -- 存储型 XSS | DVWA | Juice Shop |
| 客户端防护 -- 银行卡窃取 | CSD Demo | -- |
| 客户端防护 -- 表单劫持 | CSD Demo | -- |
| 客户端防护 -- 键盘记录 | CSD Demo | -- |
| 客户端防护 -- 加密货币挖矿 | CSD Demo | -- |
| 客户端防护 -- DOM 劫持 | CSD Demo | -- |
