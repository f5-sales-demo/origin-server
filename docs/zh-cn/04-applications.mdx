...
description: >-
  ...
  (:8888) 的 URL 路径、请求/响应行为、默认凭据和攻击场景
sidebar:
  order: 4
i18n:
...
每个应用程序以 4 个负载均衡的 Docker 容器运行，位于 nginx upstream 池后面，通过虚拟机上的 nginx 反向代理进行访问。粘性会话确保有状态应用程序（Juice Shop、DVWA、VAmPI、CSD Demo）路由的一致性。所有应用程序都是有意设计为存在漏洞的，专为安全测试而构建。

在以下所有示例中，将 `<ORIGIN>` 替换为 `http://<PUBLIC_IP>`。通过 Terraform 部署后，使用 `terraform output -raw public_ip` 获取 IP。
...
| `/dvwa/setup.php` | DVWA 初始化 | GET | 200 HTML（首次运行数据库初始化） |
...
| `/whoami/` | whoami | GET | 200 纯文本，包含主机名、IP、所有请求头 |
...
OWASP Juice Shop 是最现代且维护最活跃的漏洞 Web 应用程序。它以一个真实的电商应用程序覆盖了完整的 OWASP Top 10，包含 100 多个挑战。
...
| DOM 型 XSS | CSD | URL 片段中的载荷 |
| 认证绕过 | WAF | 暴力破解登录、JWT 篡改 |
| API 滥用 | API 安全 | 未授权访问 `/api/` 端点 |
| 敏感数据泄露 | API 安全 | 在未授权的情况下访问用户数据 |
| CSRF | WAF | 对个人资料修改的跨站请求伪造 |
...
| 场景 | 类别 | 安全级别 |
|---|---|---|
| SQL 注入 | WAF | Low：简单的 `' OR 1=1#`，High：盲注 |
...
- **Low** -- 无输入验证。所有攻击均可成功。适合 WAF 签名演示。
- **Medium** -- 基本过滤。部分攻击需要编码或绕过技术。
- **High** -- 强过滤。仅高级技术可成功。适合展示 WAF 的局限性。
- **Impossible** -- 完全安全的实现。展示正确的防御编码方式。

### 初始化设置

DVWA 在首次部署后需要一次性数据库设置：
...
|---|---|
| **路径** | `/vampi/` |
| **镜像** | `erev0s/vampi:latest`，使用 gunicorn 入口（4 个 worker） |
...
VAmPI 是专为测试 OWASP API 安全 Top 10 而构建的。它提供了一个带有故意漏洞的真实 REST API。每个实例运行带有 4 个 worker 的 gunicorn 以及独立的 SQLite 数据库。`ip_hash` 粘性会话确保来自同一客户端 IP 的注册和登录请求始终命中同一实例。

### 演示场景

| 场景 | OWASP API Top 10 | 方法 |
|---|---|---|
| 对象级授权失效（BOLA） | API1 | 通过篡改 ID 访问其他用户的数据 |
| 认证失效 | API2 | 弱令牌处理、无速率限制 |
| 过度数据暴露 | API3 | API 返回了超出客户端需要的数据 |
...
whoami 是 Traefik 的轻量级请求回显服务器。它显示源站收到的 HTTP 请求的每个细节——主机名、IP 地址、所有请求头、方法和 URL。这是验证 F5 XC 是否注入了正确请求头时最重要的诊断工具。
...
| 验证机器人防护标记 | `X-Volterra-Bot-Type`、`X-Volterra-Bot-Verified` 请求头 |
...
DVGA 是专为测试 GraphQL 特定漏洞而构建的。它在 `/dvga/` 提供了 GraphiQL IDE 用于交互式查询探索，在 `/dvga/graphql` 提供了 GraphQL API 端点。每个实例使用独立的 SQLite 数据库，因此 `ip_hash` 粘性会话确保状态的一致性。

### 演示场景

| 场景 | 类别 | 攻击向量 |
|---|---|---|
| GraphQL 注入 | API 安全 | 通过字符串拼接进行恶意查询 |
| 拒绝服务 | API 安全 | 深层嵌套查询、批量查询、资源耗尽 |
| 授权绕过 | API 安全 | 通过 GraphQL 访问未授权数据 |
| 信息泄露 | API 安全 | 内省查询泄露 Schema 详情 |
| 批量攻击 | API 安全 | 单个请求中包含多个操作 |
...
RESTaurant 是一个游戏化的漏洞 REST API，覆盖了 OWASP API 安全 Top 10 2023。它使用 FastAPI 并在 `/restaurant/docs` 提供自动生成的 Swagger UI 文档。所有 4 个实例共享一个 PostgreSQL 数据库，因此轮询负载均衡无需粘性会话即可正常工作。

### 演示场景

| 场景 | OWASP API Top 10 2023 | 方法 |
|---|---|---|
| 对象级授权失效（BOLA） | API1 | 通过篡改 ID 访问其他用户的订单 |
| 认证失效 | API2 | 弱令牌处理、凭据填充 |
| 对象属性级授权失效 | API3 | 对用户资料字段进行批量赋值 |
| 不受限的资源消耗 | API4 | 端点无速率限制 |
| 功能级授权失效（BFLA） | API5 | 以普通用户身份访问管理员端点 |
| 服务端请求伪造（SSRF） | API7 | 篡改服务端 URL 请求 |
| 安全配置错误 | API8 | 详细错误信息、默认凭据 |
...
crAPI 是 OWASP 旗舰级 API 安全测试项目。它以 7 个微服务运行在独立端口（8888）上，因为 React SPA 硬编码了 API 路径，无法在路径前缀后面提供服务。NSG 允许端口 8888 的入站流量。
...
| JWT 篡改 | 伪造或修改 JWT 令牌以提升权限 |
| NoSQL 注入 | 向 MongoDB 后端端点注入查询 |
| 过度数据暴露 | API 返回敏感用户数据 |
...
| 机器人防护 -- 暴力破解 | DVWA | Juice Shop |
| 机器人防护 -- 爬取 | Juice Shop | -- |
| 客户端防护 -- DOM XSS | Juice Shop | -- |
| 客户端防护 -- 存储型 XSS | DVWA | Juice Shop |
| 客户端防护 -- 银行卡窃取器 | CSD Demo | -- |
| 客户端防护 -- 表单劫持 | CSD Demo | -- |
| 客户端防护 -- 键盘记录器 | CSD Demo | -- |
| 客户端防护 -- 加密挖矿 | CSD Demo | -- |
