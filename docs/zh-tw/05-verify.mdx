---
title: 驗證
description: 針對所有九個應用程式端點的 curl 冒煙測試、Docker 容器健康檢查、nginx 狀態驗證、cloud-init 日誌檢查，以及常見疑難排解步驟
sidebar:
  order: 5
i18n:
...
請在等待 cloud-init 完成 5 至 10 分鐘後執行以下檢查。
...
所有端點應回傳 `200`（DVWA 會回傳 `302` 重新導向至登入頁面）。
...
此測試套件會驗證所有 9 個應用程式、健康端點、VAmPI 註冊/登入、CSD Demo 資料外洩往返、DVGA GraphQL、RESTaurant Swagger、crAPI 連接埠 8888、nginx gzip，以及版本隱藏功能。

### Docker 容器狀態

SSH 進入虛擬機器，確認所有 41 個容器均正常運行：
...
若端點回傳 502 或連線遭拒，cloud-init 可能仍在進行佈建：
...
Docker 映像約佔用 4.7 GiB 的空間。為避免在 CDN 負載測試期間磁碟空間耗盡，存取日誌記錄已停用。Logrotate 已作為縱深防禦措施進行設定（上限為 500 MiB）。請確認磁碟使用情況：
