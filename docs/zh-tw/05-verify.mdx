...
在等待 5--10 分鐘讓 cloud-init 完成後，執行以下檢查。
...
透過 SSH 連線至 VM 並確認所有 41 個容器皆在執行中：
...
如果端點回傳 502 或連線被拒，cloud-init 可能仍在佈建中：
...
Docker 映像檔約佔用 4.7 GiB。存取日誌已停用，以防止在 CDN 負載測試下磁碟空間耗盡。Logrotate 已配置為縱深防禦機制（500 MiB 上限）。驗證磁碟使用量：
