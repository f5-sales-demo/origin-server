...
在允許 cloud-init 完成的 5--10 分鐘後執行這些檢查。
...
此測試會驗證所有 9 個應用程式、健康端點、VAmPI 註冊/登入、CSD Demo exfil 往返測試、DVGA GraphQL、RESTaurant Swagger、crAPI 的 8888 連接埠、nginx gzip 以及版本隱藏。

### Docker 容器狀態

透過 SSH 連線至 VM 並驗證所有 41 個容器正在執行：
...
如果端點回傳 502 或連線被拒絕，cloud-init 可能仍在佈建中：
...
Docker 映像檔大約佔用 4.7 GiB。存取日誌已停用以防止在 CDN 負載測試下磁碟耗盡。Logrotate 已設定為縱深防禦措施（500 MiB 上限）。驗證磁碟使用量：
