---
title: API 測試指南
description: 完整的端點目錄、弱點對映及攻擊酬載，涵蓋 DVGA、RESTaurant 及 crAPI — 用於流量生成與 F5 XC API 防護設定檔開發的入門參考
sidebar:
  order: 8
i18n:
...
本指南彙整了三個 API 安全測試應用程式中的每個 API 端點、刻意設計的弱點及攻擊酬載。可用來建構流量生成模式，以開發 F5 XC API 防護設定檔。

| 應用程式 | 協定 | 路徑 | 連接埠 | 認證 | 弱點 |
|---|---|---|---|---|---|
| **DVGA** | GraphQL | `/dvga/` | 80 | 無 (admin: admin/password) | 25 個情境 |
| **RESTaurant** | REST (FastAPI) | `/restaurant/` | 80 | JWT (表單編碼) | 7 個 OWASP API 2023 分類 |
...
**管理員帳號密碼：** admin / password

所有 DVGA 的互動都透過單一端點（`/dvga/graphql`），使用包含 JSON `{"query":"..."}` 酬載的 POST 請求。
...
| 操作 | 類型 | 認證 | 用途 |
|---|---|---|---|
| `pastes(public, filter, limit)` | Query | 否 | 列出 paste（可透過 filter 進行 SQL 注入） |
| `paste(id)` | Query | 否 | 取得單一 paste |
| `me(token)` | Query | 否 | 透過 JWT 取得使用者（可偽造弱點） |
| `systemHealth` | Query | 否 | 健康檢查 |
| `systemUpdate` | Query | 否 | 慢速查詢（約 82 秒，DoS 攻擊向量） |
| `systemDiagnostics(cmd)` | Query | admin/password | 執行白名單內的作業系統指令 |
| `createPaste(title, content, public)` | Mutation | 否 | 建立 paste（content 可進行 XSS） |
...
**透過 systemDiagnostics 執行作業系統命令（需要管理員認證）：**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
**認證：** JWT，透過表單編碼的 POST 請求至 `/restaurant/token`
...
| `/restaurant/token` | POST | 否 | — | JWT 使用弱密鑰 (97952) |
| `/restaurant/healthcheck` | GET | 否 | — | 健康檢查 |
| `/restaurant/profile` | GET | 是 | 任意 | 使用者個人檔案 |
| `/restaurant/profile` | PUT | 是 | 任意 | **BOLA**（修改其他使用者） |
| `/restaurant/profile` | PATCH | 是 | 任意 | **BOPLA**（批量指派角色） |
| `/restaurant/users/update_role` | PUT | 是 | 任意 | **BFLA**（角色提權） |
| `/restaurant/menu` | GET | 是 | 任意 | 列出菜單項目 |
| `/restaurant/menu` | PUT | 是 | Employee+ | 建立菜單（透過圖片進行 **SSRF**） |
| `/restaurant/menu/{item_id}` | PUT | 是 | Employee+ | 更新菜單（透過圖片進行 **SSRF**） |
| `/restaurant/menu/{item_id}` | DELETE | 是 | 任意 | **BFLA**（任何使用者都可刪除） |
| `/restaurant/orders` | GET | 是 | 任意 | **BOLA**（檢視所有訂單） |
...
| `/restaurant/referral-code` | GET | 是 | 任意 | 取得推薦代碼 |
| `/restaurant/apply-referral` | POST | 是 | 任意 | 套用推薦 |
| `/restaurant/discount-coupons` | GET | 是 | 任意 | 列出優惠券 |

### 弱點目錄

#### API1:2023 — 物件層級授權破壞 (BOLA)
...
#### API3:2023 — 物件屬性層級授權破壞 (BOPLA)

**批量指派角色，從 Customer 提升為 Chef：**
```bash
...
#### API5:2023 — 功能層級授權破壞 (BFLA)

**以 Customer 身份刪除菜單項目（應需要 Employee 以上權限）：**
```bash
...
**Web UI：** `${CRAPI}/`
**MailHog：** `${CRAPI}/mailhog/`（用於擷取驗證信件）
**認證：** JWT Bearer 權杖（RS256，存在演算法混淆弱點）
...
# 2. 檢查 MailHog 的驗證信件
# 瀏覽至 ${CRAPI}/mailhog/ 或使用 MailHog API：
...
| `/identity/api/v2/user/dashboard` | GET | 是 | 使用者個人檔案 |
| `/identity/api/v2/user/change-email` | PUT | 是 | 變更信箱 |
...
| `/identity/api/v2/admin/videos/{id}` | DELETE | 是 | **BFLA**（無管理員檢查） |
...
| 端點 | 方法 | 認證 | 弱點 |
|---|---|---|---|
| `/workshop/api/mechanic` | GET | 是 | **資料洩露**（技師信箱） |
| `/workshop/api/mechanic/mechanic_report` | GET | 否 | **BOLA**（無認證、循序 ID） |
...
| `/workshop/api/shop/orders/{id}` | GET | 否 | **BOLA**（不需認證） |
...
#### 挑戰 8 與 9 — 批量指派：透過訂單操作取得免費商品

```bash
# 將 GET 改為 PUT，修改 status 和 quantity
...
## OWASP API 安全 Top 10 交叉對照

| OWASP 分類 | DVGA | RESTaurant | crAPI |
|---|---|---|---|
| **API1: BOLA** | — | `PUT /profile`、`GET /orders/{id}` | 車輛位置、技師報告、訂單 |
| **API2: 認證破壞** | JWT 偽造（`me` 查詢） | JWT 弱密鑰 (97952) | OTP 暴力破解（v2）、JWT 演算法混淆 |
| **API3: BOPLA** | — | `PATCH /profile`（角色提權） | 影片 conversion_params、技師信箱洩露 |
| **API4: 資源消耗** | 批次、遞迴、別名、欄位重複 DoS | 透過回應長度列舉使用者 | `contact_mechanic` repeat_request DoS |
| **API5: BFLA** | — | `DELETE /menu`、`PUT /users/update_role` | 以一般使用者身份 `DELETE /admin/videos` |
...
| **API8: 注入** | SQL（`filter`）、XSS（`content`）、OS cmd（`importPaste`、`systemDiagnostics`） | OS cmd（`/admin/stats/disk?parameters=`） | NoSQL（`validate-coupon`）、SQL（`apply_coupon`） |
| **API9: 資產管理不當** | — | — | 已棄用的 `/auth/v2/check-otp`（無速率限制） |
| **API10: 不安全的消費** | — | — | （透過 SSRF 間接觸發） |
| **GraphQL 特定** | 自省、批次、遞迴、別名、欄位重複、循環片段 | — | — |
...
### 階段 2：依 OWASP 分類的攻擊流量

**BOLA 測試：** 列舉循序 ID、在請求中交換使用者識別碼、在無所有權的情況下存取資源。

**注入測試：** 在 filter/coupon 參數中使用 SQL 酬載、GraphQL 查詢注入、透過參數欄位進行作業系統命令注入。

**認證繞過測試：** 偽造 JWT 權杖、暴力破解 OTP、使用已棄用的 API 版本、演算法混淆。

**SSRF 測試：** 在 `importPaste`、`image_url` 及 `mechanic_api` 參數中使用內部 URL。

**DoS 測試：** GraphQL 批次/遞迴/別名攻擊、`repeat_request_if_failed` 搭配高重複次數。

### 階段 3：有狀態的攻擊鏈

某些攻擊需要具有狀態的多步驟序列：

1. **RESTaurant 提權鏈：** 註冊 → 取得權杖 → PATCH 角色為 Chef → 重新認證 → 命令注入
2. **crAPI 完整流程：** 註冊 → 驗證信箱（MailHog）→ 登入 → 新增車輛 → 聯絡技師（SSRF）→ 訂單操作
3. **DVGA 偵察到利用：** 自省查詢 → 發現 systemDiagnostics → 暴力破解管理員帳號密碼 → 作業系統命令執行
...
| BOLA 列舉 | 100-500 | 2 分鐘 | 循序 ID 掃描 |
| OTP 暴力破解 | 1000+ | 直到找到為止 | 最多 10,000 次嘗試（4 位數） |
| GraphQL DoS | 10-50 | 30 秒 | 每個請求在伺服器端消耗大量資源 |
| 注入模糊測試 | 50-200 | 5 分鐘 | 每個請求變更酬載 |
| SSRF 探測 | 5-20 | 2 分鐘 | 較慢；每個請求都會觸發伺服器端的 HTTP 請求 |
