---
title: API 測試指南
description: 完整的端點目錄、弱點對應與攻擊酬載，涵蓋 DVGA、RESTaurant 與 crAPI — 用於流量生成與 F5 XC API 防護設定檔開發的入門參考
sidebar:
  order: 8
i18n:
...
本指南收錄了三個 API 安全測試應用程式中的所有 API 端點、刻意設計的弱點及攻擊酬載。可用於建立流量生成模式，以協助 F5 XC API 防護設定檔的開發。
...
所有 DVGA 互動皆使用單一端點（`/dvga/graphql`），透過包含 JSON `{"query":"..."}` 酬載的 POST 請求進行。
...
Types:    PasteObject (id, title, content, public, owner, ipAddr, userAgent)
          OwnerObject (id, username, pastes)  ← 循環參考
```
...
| 操作 | 類型 | 驗證 | 用途 |
|---|---|---|---|
| `pastes(public, filter, limit)` | Query | 否 | 列出 paste（filter 可進行 SQL 注入） |
| `paste(id)` | Query | 否 | 取得單一 paste |
| `me(token)` | Query | 否 | 透過 JWT 取得使用者（可偽造） |
| `systemHealth` | Query | 否 | 健康檢查 |
| `systemUpdate` | Query | 否 | 慢速查詢（約 82 秒，DoS 向量） |
| `systemDiagnostics(cmd)` | Query | admin/password | 執行白名單中的作業系統指令 |
| `createPaste(title, content, public)` | Mutation | 否 | 建立 paste（content 可進行 XSS） |
| `importPaste(host, port, path, scheme)` | Mutation | 否 | 匯入遠端 paste（SSRF、指令注入） |
...
**透過 systemDiagnostics 執行作業系統指令（需要管理員驗證）：**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
| `/restaurant/profile` | PATCH | 是 | 任何 | **BOPLA**（大量指派角色） |
| `/restaurant/users/update_role` | PUT | 是 | 任何 | **BFLA**（角色提權） |
| `/restaurant/menu` | GET | 是 | 任何 | 列出菜單項目 |
| `/restaurant/menu` | PUT | 是 | Employee+ | 建立菜單（透過 image 進行 **SSRF**） |
| `/restaurant/menu/{item_id}` | PUT | 是 | Employee+ | 更新菜單（透過 image 進行 **SSRF**） |
...
**以 Customer 身分刪除菜單項目（應需要 Employee+ 權限）：**
```bash
...
**驗證：** JWT Bearer 令牌（RS256，存在演算法混淆弱點）
...
| `/identity/api/auth/login` | POST | 否 | JWT 令牌（演算法混淆） |
...
| `/workshop/api/shop/products` | GET | 是 | 商品目錄 |
| `/workshop/api/shop/orders/` | POST | 是 | 建立訂單 |
| `/workshop/api/shop/orders/all` | GET | 是 | 列出訂單 |
| `/workshop/api/shop/orders/{id}` | GET | 否 | **BOLA**（不需驗證） |
...
## OWASP API 安全前 10 名交叉參考

| OWASP 類別 | DVGA | RESTaurant | crAPI |
|---|---|---|---|
| **API1: BOLA** | — | `PUT /profile`、`GET /orders/{id}` | 車輛位置、技師報告、訂單 |
| **API2: 驗證機制失效** | JWT 偽造（`me` 查詢） | JWT 弱密鑰（97952） | OTP 暴力破解（v2）、JWT 演算法混淆 |
...
| **API5: BFLA** | — | `DELETE /menu`、`PUT /users/update_role` | 以一般使用者 `DELETE /admin/videos` |
...
| **API8: 注入攻擊** | SQL（`filter`）、XSS（`content`）、OS cmd（`importPaste`、`systemDiagnostics`） | OS cmd（`/admin/stats/disk?parameters=`） | NoSQL（`validate-coupon`）、SQL（`apply_coupon`） |
| **API9: 不當資產管理** | — | — | 已棄用的 `/auth/v2/check-otp`（無速率限制） |
| **API10: 不安全的使用** | — | — | （透過 SSRF 間接利用） |
| **GraphQL 特有** | 自省查詢、批次、遞迴、別名、欄位重複、循環片段 | — | — |
...
在測試攻擊模式之前，先產生合法流量以建立正常的 API 行為基線。
...
### 階段 2：依 OWASP 類別的攻擊流量

**BOLA 測試：** 列舉循序 ID、在請求中交換使用者識別碼、在無擁有權的情況下存取資源。

**注入測試：** 在 filter/coupon 參數中使用 SQL 酬載、GraphQL 查詢注入、透過參數欄位進行作業系統指令注入。

**驗證繞過測試：** 偽造 JWT 令牌、暴力破解 OTP、使用已棄用的 API 版本、演算法混淆。

**SSRF 測試：** 在 `importPaste`、`image_url` 和 `mechanic_api` 參數中使用內部 URL。

**DoS 測試：** GraphQL 批次/遞迴/別名攻擊、`repeat_request_if_failed` 搭配高次數。

### 階段 3：有狀態的攻擊鏈

某些攻擊需要具有狀態的多步驟序列：

1. **RESTaurant 提權鏈：** 註冊 → 取得令牌 → PATCH 角色為 Chef → 重新驗證 → 指令注入
2. **crAPI 完整流程：** 註冊 → 驗證電子郵件（MailHog） → 登入 → 新增車輛 → 聯繫技師（SSRF） → 訂單操控
3. **DVGA 偵查到利用：** 自省查詢 → 發現 systemDiagnostics → 暴力破解管理員憑證 → 作業系統指令執行

### 速率建議

| 模式 | 請求數/秒 | 持續時間 | 備註 |
|---|---|---|---|
| 基線（每個應用程式） | 10-50 | 5 分鐘 | 建立正常流量特徵 |
| BOLA 列舉 | 100-500 | 2 分鐘 | 循序 ID 掃描 |
| OTP 暴力破解 | 1000+ | 直到找到為止 | 最多 10,000 次嘗試（4 位數） |
| GraphQL DoS | 10-50 | 30 秒 | 每個請求在伺服器端消耗大量資源 |
| 注入模糊測試 | 50-200 | 5 分鐘 | 每次請求使用不同酬載 |
