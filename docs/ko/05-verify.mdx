...
cloud-init이 완료될 때까지 5~10분을 기다린 후 다음 검사를 실행하십시오.
...
모든 엔드포인트는 `200`을 반환해야 합니다 (DVWA는 로그인 페이지로의 `302` 리다이렉트를 반환).
...
이 테스트는 9개의 모든 애플리케이션, 헬스 엔드포인트, VAmPI 등록/로그인, CSD Demo 유출 왕복 테스트, DVGA GraphQL, RESTaurant Swagger, 포트 8888의 crAPI, nginx gzip, 그리고 버전 숨김을 검증합니다.

### Docker 컨테이너 상태

VM에 SSH로 접속하여 41개의 모든 컨테이너가 실행 중인지 확인합니다:
...
- `status: done` -- 프로비저닝이 완료되었으므로 Docker 컨테이너를 확인하십시오
...
Docker 이미지는 약 4.7 GiB를 사용합니다. CDN 부하 테스트 시 디스크 고갈을 방지하기 위해 접근 로깅이 비활성화되어 있습니다. Logrotate가 심층 방어 차원에서 설정되어 있습니다 (500 MiB 상한). 디스크 사용량을 확인하십시오:
...
메모리 한도에 근접한 컨테이너가 있는지 확인합니다 (OOM 위험):
