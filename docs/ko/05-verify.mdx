...
title: 검증
description: >-
  9개 애플리케이션 엔드포인트 전체에 대한 curl 스모크 테스트, Docker 컨테이너 상태 확인, nginx 상태 검증,
  cloud-init 로그 점검, 일반적인 문제 해결 단계
sidebar:
  order: 5
i18n:
...
cloud-init이 완료될 때까지 5~10분을 기다린 후 다음 확인을 수행하십시오.
...
모두 `200`을 반환해야 합니다(DVWA는 로그인 페이지로의 `302` 리다이렉트를 반환합니다).

### 자동화된 스모크 테스트

리포지토리에 39개 항목의 스모크 테스트 스위트가 포함되어 있습니다:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

이 테스트는 9개 애플리케이션 전체, 헬스 엔드포인트, VAmPI 등록/로그인, CSD Demo exfil 왕복 테스트, DVGA GraphQL, RESTaurant Swagger, 포트 8888의 crAPI, nginx gzip, 버전 숨김을 검증합니다.
...
업스트림 컨테이너가 준비되지 않았거나 크래시된 상태입니다:
...
Docker 이미지는 약 4.7 GiB를 사용합니다. CDN 부하 테스트 중 디스크 고갈을 방지하기 위해 접근 로깅은 비활성화되어 있습니다. Logrotate는 심층 방어 차원에서 설정되어 있습니다(500 MiB 제한). 디스크 사용량을 확인하십시오:
...
메모리 제한에 근접한 컨테이너가 있는지 확인합니다(OOM 위험):
