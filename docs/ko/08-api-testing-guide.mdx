...
title: API 테스트 가이드
description: >-
  DVGA, RESTaurant, crAPI를 위한 종합 엔드포인트 카탈로그, 취약점 매핑 및 공격 페이로드 — 트래픽 생성 및 F5 XC
  API 보호 프로필 개발을 위한 온보딩 참조 문서
sidebar:
  order: 8
i18n:
...
이 가이드는 세 가지 API 보안 테스트 애플리케이션의 모든 API 엔드포인트, 의도적 취약점 및 공격 페이로드를 카탈로그화합니다. F5 XC API 보호 프로필 개발을 위한 트래픽 생성 패턴을 구축하는 데 활용하십시오.

| 애플리케이션 | 프로토콜 | 경로 | 포트 | 인증 | 취약점 |
|---|---|---|---|---|---|
| **DVGA** | GraphQL | `/dvga/` | 80 | 없음 (관리자: admin/password) | 25개 시나리오 |
| **RESTaurant** | REST (FastAPI) | `/restaurant/` | 80 | JWT (폼 인코딩) | OWASP API 2023 7개 카테고리 |
| **crAPI** | REST (마이크로서비스) | `/` | 8888 | JWT (Bearer) | 18개 이상의 챌린지 |
...
모든 DVGA 상호작용은 단일 엔드포인트(`/dvga/graphql`)에 JSON `{"query":"..."}` 페이로드를 포함한 POST 요청을 사용합니다.
...
**OpenAPI 스펙:** `${ORIGIN}/restaurant/openapi.json`
**인증:** 폼 인코딩 POST를 `/restaurant/token`으로 전송하여 JWT 획득
...
| `/restaurant/healthcheck` | GET | 아니오 | — | 상태 확인 |
| `/restaurant/profile` | GET | 예 | 모두 | 사용자 프로필 |
| `/restaurant/profile` | PUT | 예 | 모두 | **BOLA** (다른 사용자 수정) |
| `/restaurant/profile` | PATCH | 예 | 모두 | **BOPLA** (역할 대량 할당) |
| `/restaurant/users/update_role` | PUT | 예 | 모두 | **BFLA** (역할 상승) |
| `/restaurant/menu` | GET | 예 | 모두 | 메뉴 항목 목록 |
| `/restaurant/menu` | PUT | 예 | Employee+ | 메뉴 생성 (이미지를 통한 **SSRF**) |
| `/restaurant/menu/{item_id}` | PUT | 예 | Employee+ | 메뉴 업데이트 (이미지를 통한 **SSRF**) |
| `/restaurant/menu/{item_id}` | DELETE | 예 | 모두 | **BFLA** (모든 사용자가 삭제 가능) |
| `/restaurant/orders` | GET | 예 | 모두 | **BOLA** (모든 주문 조회) |
| `/restaurant/orders` | POST | 예 | 모두 | 주문 생성 |
| `/restaurant/orders/{order_id}` | GET | 예 | 모두 | **BOLA** (다른 주문 접근) |
| `/restaurant/orders/status/{order_id}` | GET | 예 | 모두 | 주문 상태 |
...
| `/restaurant/referral-code` | GET | 예 | 모두 | 추천 코드 조회 |
| `/restaurant/apply-referral` | POST | 예 | 모두 | 추천 적용 |
| `/restaurant/discount-coupons` | GET | 예 | 모두 | 쿠폰 목록 |

### 취약점 카탈로그

#### API1:2023 — 객체 수준 권한 부여 결함 (BOLA)
...
# 시크릿: 97952 (hashcat -a 3 -m 16500 token '?d?d?d?d?d?d'로 무차별 대입 가능)
...
#### API3:2023 — 객체 속성 수준 권한 부여 결함 (BOPLA)
...
# ${CRAPI}/mailhog/으로 이동하거나 MailHog API 사용:
...
| `/identity/api/v2/vehicle/vehicles` | GET | 예 | 차량 목록 (UUID 유출) |
| `/identity/api/v2/vehicle/{uuid}/location` | GET | 예 | **BOLA** (임의 사용자의 차량) |
...
| 엔드포인트 | 메서드 | 인증 | 취약점 |
|---|---|---|---|
| `/community/api/v2/community/posts` | GET | 예 | **데이터 노출** (vehicle_id, email 유출) |
| `/community/api/v2/community/posts` | POST | 예 | 블로그 게시물 생성 |
...
```bash
# Authorization 헤더 없음 — 여전히 데이터 반환
for i in 1 2 3 4 5; do
...
# 1. 헤더 설정: {"alg":"none","typ":"JWT"}를 base64url 인코딩
# 2. 페이로드 설정: {"email":"admin@example.com","role":"admin"}를 base64url 인코딩
# 3. 점으로 연결하고 서명을 비워 둠: <header>.<payload>.
...
|---|---|---|---|
| **API1: BOLA** | — | `PUT /profile`, `GET /orders/{id}` | 차량 위치, 정비사 리포트, 주문 |
| **API2: 인증 결함** | JWT 위조 (`me` 쿼리) | JWT 약한 시크릿 (97952) | OTP 무차별 대입 (v2), JWT 알고리즘 혼동 |
| **API3: BOPLA** | — | `PATCH /profile` (역할 상승) | 비디오 conversion_params, 정비사 이메일 노출 |
...
| **API7: SSRF** | `importPaste` 뮤테이션 | `PUT /menu` image_url | `contact_mechanic` mechanic_api URL |
...
| **API9: 부적절한 자산 관리** | — | — | 더 이상 사용되지 않는 `/auth/v2/check-otp` (속도 제한 없음) |
...
공격 패턴을 테스트하기 전에 정상적인 API 동작을 확립하기 위한 합법적인 트래픽을 생성합니다.
...
**BOLA 테스트:** 순차 ID 열거, 요청에서 사용자 식별자 교체, 소유권 없이 리소스 접근.

**인젝션 테스트:** filter/coupon 매개변수의 SQL 페이로드, GraphQL 쿼리 인젝션, 매개변수 필드를 통한 OS 명령 인젝션.

**인증 우회 테스트:** JWT 토큰 위조, OTP 무차별 대입, 더 이상 사용되지 않는 API 버전 사용, 알고리즘 혼동.
...
일부 공격은 상태를 유지하는 다단계 시퀀스가 필요합니다:

1. **RESTaurant 상승 체인:** 등록 → 토큰 → PATCH로 Chef 역할 → 재인증 → 명령 인젝션
2. **crAPI 전체 흐름:** 가입 → 이메일 인증 (MailHog) → 로그인 → 차량 추가 → 정비사 연락 (SSRF) → 주문 조작
3. **DVGA 정찰에서 익스플로잇까지:** 인트로스펙션 → systemDiagnostics 발견 → 관리자 자격 증명 무차별 대입 → OS 명령 실행

### 속도 권장 사항

| 패턴 | 요청/초 | 지속 시간 | 비고 |
|---|---|---|---|
| 기준선 (앱별) | 10-50 | 5분 | 정상 트래픽 핑거프린트 확립 |
| BOLA 열거 | 100-500 | 2분 | 순차 ID 스캐닝 |
| OTP 무차별 대입 | 1000+ | 발견될 때까지 | 최대 10,000회 시도 (4자리) |
| GraphQL DoS | 10-50 | 30초 | 각 요청이 서버 측에서 비용이 큼 |
