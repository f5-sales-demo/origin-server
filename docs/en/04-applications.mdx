...
This repository provides two distinct origins; they are not feature-equivalent:

- **Azure full-origin:** a VM running nginx in front of 41 containers for all nine applications and the path-based URLs documented below. Set `ORIGIN` from `terraform output -raw origin_url` in `terraform/`.
- **AWS Juice Shop:** private Fargate tasks behind an Application Load Balancer (ALB). It serves Juice Shop directly at the ALB root (`/`), not at `/juice-shop/`. Set `ORIGIN` from the consuming configuration's module output, for example `terraform output -raw origin_url`.
  The ALB is internal by default, so that URL resolves and responds only from an authorized routed network unless public exposure was explicitly configured.

The AWS module does not deploy nginx, a container host, or the other eight applications. Every path in the quick reference below, including non-Juice-Shop APIs, requires the Azure full-origin unless stated otherwise.

## Azure Full-Origin URL Reference
...
| Offering | URL behavior | Runtime |
| --- | --- | --- |
| Azure full-origin | `${ORIGIN}/juice-shop/` | Four Docker containers behind nginx with sticky routing and a 60-second proxy-cache TTL |
| AWS Juice Shop | `${ORIGIN}/` | Private Fargate task or tasks behind an ALB; no nginx or host SSH |

For Azure, obtain `ORIGIN` from the root configuration:

```bash
ORIGIN=$(terraform -chdir=terraform output -raw origin_url)
curl -sf "${ORIGIN}/juice-shop/" -o /dev/null -w "%{http_code}\n"
```

For AWS, expose the module's `origin_url` from the consuming root configuration and use it without a `/juice-shop/` prefix:
...
curl -sf "${ORIGIN}/" -o /dev/null -w "%{http_code}\n"
curl -sf "${ORIGIN}/rest/products/search?q=" | jq .
```

### Juice Shop demo scenarios
...
IP: <container-loopback-ip>
RemoteAddr: <immediate-peer-ip>:<ephemeral-port>
GET / HTTP/1.1
Host: <origin-hostname-or-ip>
User-Agent: curl/<version>
Accept: */*
True-Client-Ip: <simulated-client-ip>
X-Forwarded-For: <simulated-client-ip>, <trusted-proxy-ip>
X-Forwarded-Proto: https
X-Real-Ip: <observed-client-ip>
```
