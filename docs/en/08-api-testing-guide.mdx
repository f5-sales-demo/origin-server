...
This guide covers DVGA, RESTaurant, and crAPI in the **Azure full-origin** deployment. Those applications and their path-based routes are not deployed by `terraform/modules/aws-juice-shop`; all non-Juice-Shop APIs require Azure full-origin.

The AWS offering contains only Juice Shop on private Fargate behind an ALB and serves it at the module `origin_url` root (`/`). For a Juice Shop API smoke test on AWS, expose `module.juice_shop.origin_url`, then run `curl -sf "${ORIGIN}/rest/products/search?q="`. Do not use the `/juice-shop/` prefix on AWS.

This guide catalogs every API endpoint, intentional vulnerability, and attack payload across the three Azure API security testing applications. Use it to build traffic generation patterns for F5 XC API protection profile development.
...
CRAPI=$(terraform -chdir=terraform output -raw crapi_url)
```
...
### RESTaurant vulnerability catalog
