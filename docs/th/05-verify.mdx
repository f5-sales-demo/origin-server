...
## การทดสอบ Smoke Tests

รันการตรวจสอบเหล่านี้หลังจากรอให้ cloud-init ทำงานเสร็จสมบูรณ์ประมาณ 5--10 นาที
...
ทั้งหมดควรส่งคืน `200` (DVWA จะส่งคืน `302` redirect ไปยังหน้า login)

### Smoke Test อัตโนมัติ

ใน repository มีชุดทดสอบ smoke test 39 รายการ:

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

ชุดทดสอบนี้จะตรวจสอบแอปพลิเคชันทั้ง 9 ตัว, health endpoints, การลงทะเบียนและเข้าสู่ระบบ VAmPI, การทดสอบ CSD Demo exfil round-trip, DVGA GraphQL, RESTaurant Swagger, crAPI บนพอร์ต 8888, nginx gzip และการซ่อนเวอร์ชัน
...
ผลลัพธ์ที่คาดหวัง (41 containers):
...
หาก endpoint ส่งคืน 502 หรือ connection refused แสดงว่า cloud-init อาจยังอยู่ระหว่างการ provisioning:

```bash
ssh azureuser@${ORIGIN_IP} "cloud-init status"
```

- `status: running` -- ยังอยู่ระหว่างการ provisioning ให้รอสักครู่
- `status: done` -- การ provisioning เสร็จสมบูรณ์แล้ว ให้ตรวจสอบ Docker containers
- `status: error` -- ตรวจสอบ logs ด้านล่าง
...
### Container ไม่สามารถเริ่มทำงานได้
...
upstream container ยังไม่พร้อมหรือเกิด crash:
...
Docker images ใช้พื้นที่ประมาณ 4.7 GiB การบันทึก access log ถูกปิดใช้งานเพื่อป้องกันดิสก์เต็มระหว่างการทดสอบ CDN load Logrotate ได้รับการกำหนดค่าเป็นการป้องกันเชิงลึก (จำกัดที่ 500 MiB) ตรวจสอบการใช้งานดิสก์:
...
ตรวจสอบ container ที่เข้าใกล้ขีดจำกัดหน่วยความจำ (เสี่ยง OOM):
