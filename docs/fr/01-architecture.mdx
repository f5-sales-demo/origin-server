...
description: >-
  ...
  vers des instances réparties en charge de Juice Shop, DVWA-FPM, VAmPI,
...
- **Route par préfixe de chemin** vers des pools upstream répartis en charge (4 instances par application)
...
- **Journalisation des accès désactivée** pour éviter l'épuisement du disque lors des tests de charge CDN (logrotate en défense en profondeur)
- **Transmission des en-têtes client** (`X-Real-IP`, `X-Forwarded-For`, `X-Forwarded-Proto`) pour la visibilité de l'origine
...
| `/vampi/` | vampi | 4 | 5101-5104 | `ip_hash` | Tests de sécurité API REST (OWASP API Top 10) |
| `/httpbin/` | httpbin_up | 4 | 8201-8204 | -- | Service de requête/réponse HTTP pour les démonstrations API |
...
| `/dvga/` | dvga | 4 | 5201-5204 | `ip_hash` | Tests de sécurité API GraphQL (injection, DoS, contournement d'authentification) |
| `/restaurant/` | restaurant | 4 + PostgreSQL | 8301-8304 | -- | Sécurité API REST (OWASP API Top 10 2023) |
...
- **Le simulateur CDN** fournit la couche edge CDN (mise en cache nginx sur une VM Azure)
...
| **DVWA** | Standard de l'industrie pour les tests WAF ; niveaux de sécurité ajustables (low/medium/high/impossible) ; build personnalisé php-fpm + nginx pour la performance ; backend MariaDB 10.11 partagé |
| **VAmPI** | Conçu spécifiquement pour le Top 10 OWASP de la sécurité API ; API REST avec des vulnérabilités connues ; gunicorn avec 4 workers par instance ; session persistante ip_hash pour la cohérence SQLite |
| **httpbin** | Service de test HTTP canonique de Kenneth Reitz ; gunicorn avec 4 workers gevent ; utile pour les démonstrations API et l'inspection des requêtes |
| **whoami** | Serveur d'écho de requête de Traefik ; affiche les détails complets de la requête tels que l'origine les voit — essentiel pour vérifier l'injection d'en-têtes F5 XC |
| **CSD Demo** | Page de paiement personnalisée avec 5 attaques de type Magecart activables/désactivables (skimmer de carte, formjacker, keylogger, cryptominer, détournement DOM) ; point de terminaison d'exfiltration + tableau de bord attaquant ; gunicorn mono-worker pour la persistance de l'état en mémoire |
| **DVGA** | Damn Vulnerable GraphQL Application ; vulnérabilités spécifiques à GraphQL incluant injection, DoS, attaques par lot et contournement d'autorisation ; interface GraphiQL pour l'exploration interactive ; session persistante ip_hash pour SQLite par instance |
| **RESTaurant** | Damn Vulnerable RESTaurant API Game ; conçu spécifiquement pour le Top 10 OWASP de la sécurité API 2023 ; FastAPI avec interface Swagger ; backend PostgreSQL 15.4 partagé ; couvre BOLA, BFLA, assignation de masse, SSRF et injection |
