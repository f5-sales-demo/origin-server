...
title: Vérification
description: >-
  Tests curl de validation pour les neuf points de terminaison applicatifs,
  vérifications de l'état des conteneurs Docker, vérification du statut nginx,
...
Le dépôt inclut une suite de tests de validation en 39 points :

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

Cela valide les 9 applications, les points de terminaison de santé, l'inscription/connexion VAmPI, le cycle complet d'exfiltration CSD Demo, GraphQL DVGA, Swagger RESTaurant, crAPI sur le port 8888, la compression gzip de nginx et le masquage de version.
...
### Cloud-init encore en cours d'exécution

Si les points de terminaison retournent 502 ou une connexion refusée, cloud-init peut encore être en cours de provisionnement :

```bash
ssh azureuser@${ORIGIN_IP} "cloud-init status"
```

- `status: running` -- provisionnement encore en cours, patientez quelques minutes
...
Vérifiez si des conteneurs approchent de leurs limites mémoire (risque OOM) :
