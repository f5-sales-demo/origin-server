...
Exécutez ces vérifications après avoir attendu 5 à 10 minutes pour que cloud-init termine son exécution.
...
### Points de terminaison des applications
...
Le dépôt inclut une suite de 39 tests de vérification :

```bash
./tests/smoke-test.sh ${ORIGIN_IP}
```

Cela valide les 9 applications, les points de terminaison de santé, l'inscription/connexion VAmPI, le cycle complet d'exfiltration CSD Demo, DVGA GraphQL, RESTaurant Swagger, crAPI sur le port 8888, la compression gzip de nginx et le masquage de version.
